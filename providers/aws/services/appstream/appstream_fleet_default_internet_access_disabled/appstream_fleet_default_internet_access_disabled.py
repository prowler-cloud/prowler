    ...
    def execute(self):
        ...
        for fleet in appstream_client.fleets:
            ...
            if fleet.enable_default_internet_access:
                report.status = "FAIL"
                report.status_extended = f"Fleet {fleet.name} has default internet access enabled"
            else:
                report.status = "PASS"
                report.status_extended = f"Fleet {fleet.name} has default internet access disabled"
