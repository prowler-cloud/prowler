    ...
    def execute(self):
        ...
        for rest_api in apigateway_client.rest_apis:
            ...
            for stage in rest_api.stages:
                if stage.waf:
                    report.status = "PASS"
                    report.status_extended = f"API Gateway {rest_api.name} ID {rest_api.id} in {stage.name} has {stage.waf} WAF ACL attached."
                    report.resource_id = rest_api.name
                else:
                    report.status = "FAIL"
                    report.status_extended = f"API Gateway {rest_api.name} ID {rest_api.id} in {stage.name} has not WAF ACL attached."
                    report.resource_id = rest_api.name
