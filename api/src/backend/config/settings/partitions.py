...
# Set the months for each partition. Setting the partition months to 1 will create partitions with a size of 1 natural month.
API_KEY_ACTIVITY_TABLE_PARTITION_MONTHS = env.int(
    "API_KEY_ACTIVITY_TABLE_PARTITION_MONTHS", 1
)

# Set the number of partitions to create
API_KEY_ACTIVITY_TABLE_PARTITION_COUNT = env.int(
    "API_KEY_ACTIVITY_TABLE_PARTITION_COUNT", 13
)
...
):  # Only override if explicitly set to non-default
    API_KEY_ACTIVITY_TABLE_PARTITION_MAX_AGE_MONTHS = math.ceil(
        API_KEY_ACTIVITY_RETENTION_DAYS / 30.44
