...
from api.models import Finding, Provider, Task
...
def generate_threatscore_report_job(tenant_id: str, scan_id: str, provider_id: str):
    ...
    out_dir, _ = _generate_output_directory(
        DJANGO_TMP_OUTPUT_DIRECTORY, provider_uid, tenant_id, scan_id
    )
    pdf_path = f"{out_dir}_threatscore_report.pdf"
    ...
    if os.path.exists(pdf_path):
        try:
            s3_uri = _upload_to_s3(tenant_id, pdf_path, scan_id)
        except Exception as e:
            logger.error(f"Error uploading PDF to S3: {e}")

    try:
        task = Task.objects.get(scan_id=scan_id)
        result_path = s3_uri if s3_uri else pdf_path
        task.result = result_path
        task.save(update_fields=["result"])
    except Exception as e:
        logger.error(f"Error saving PDF location to Task: {e}")
    return {"pdf_location": s3_uri if s3_uri else pdf_path}
