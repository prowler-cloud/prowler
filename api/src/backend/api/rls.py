...
class APIKeyRowLevelSecurityConstraint(models.BaseConstraint):
    ...
    # Permissive policy template for API keys - allows access when no tenant context for SELECT
    # but still restricts INSERT/UPDATE/DELETE to require proper tenant context
    policy_sql_query = """
        CREATE POLICY %(db_user)s_%(table_name)s_auth_{statement}
        ON %(table_name)s
        FOR {statement}
        TO %(db_user)s
        {clause} (
            CASE
                WHEN current_setting('%(tenant_setting)s', True) IS NULL THEN {allow_no_context}
                ELSE %(field_column)s = current_setting('%(tenant_setting)s')::uuid
            END
        );
    """
    ...
    drop_policy_sql_query = """
        DROP POLICY IF EXISTS %(db_user)s_%(raw_table_name)s_auth_{statement} ON %(table_name)s;
    """
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, APIKeyRowLevelSecurityConstraint):
            return self.name == other.name and self.target_field == other.target_field
        return super().__eq__(other)
