...
class ProviderSecretFilter(FilterSet):
    ...
    updated_at = DateFilter(
        field_name="updated_at",
        lookup_expr="date",
        help_text="Filter by exact update date (format: YYYY-MM-DD)",
    )
    provider = UUIDFilter(
        field_name="provider__id",
        lookup_expr="exact",
        help_text="Prowler provider ID (UUIDv4 generated by Prowler)",
    )
    name = CharFilter(
        field_name="name", lookup_expr="exact", help_text="Filter by exact secret name"
    )
    name__icontains = CharFilter(
        field_name="name",
        lookup_expr="icontains",
        help_text="Filter by partial secret name match",
    )
