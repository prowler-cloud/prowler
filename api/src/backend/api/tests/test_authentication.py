...
class TestAPIKeyAuthentication:
    ...
    def test_authenticate_malformed_header_no_space(self, auth_instance, request_factory):
    ...
    def test_authenticate_case_insensitive_keyword(self, auth_instance, request_factory, valid_api_key):
    ...
    def test_authenticate_credentials_invalid_format(self, auth_instance, request_factory):
    ...
    def test_authenticate_credentials_nonexistent_key(self, auth_instance, request_factory):
    ...
    def test_authenticate_credentials_wrong_key(self, auth_instance, request_factory, valid_api_key):
    ...
    def test_authenticate_credentials_expired_key(self, auth_instance, request_factory, expired_api_key):
        """Test authenticate_credentials with expired key."""
        request = request_factory.get("/api/v1/test")
        
        with pytest.raises(exceptions.AuthenticationFailed, match="API key has expired"):
            auth_instance.authenticate_credentials(expired_api_key._raw_key, request)

    def test_authenticate_credentials_revoked_key(self, auth_instance, request_factory, revoked_api_key):
        """Test authenticate_credentials with revoked key."""
        request = request_factory.get("/api/v1/test")
        
        with pytest.raises(exceptions.AuthenticationFailed, match="API key has been revoked"):
            auth_instance.authenticate_credentials(revoked_api_key._raw_key, request)

    def test_authenticate_credentials_valid_key(self, auth_instance, request_factory, valid_api_key):
        """Test successful authentication with valid key."""
        request = request_factory.get("/api/v1/test")
        
        user, auth_info = auth_instance.authenticate_credentials(valid_api_key._raw_key, request)
    ...
    def test_authenticate_credentials_updates_last_used(self, auth_instance, request_factory, valid_api_key):
    ...
    def test_authenticate_full_flow_valid_key(self, auth_instance, request_factory, valid_api_key):
    ...
    def test_authenticate_with_multiple_candidate_keys(self, auth_instance, request_factory, tenant):
    ...
    def test_authenticate_credentials_timezone_mock(self, auth_instance, request_factory, valid_api_key):
        ...
        assert (timezone.now() - valid_api_key.last_used_at).total_seconds() < 5  # Within 5 seconds

    def test_authenticate_credentials_database_save_fields(self, auth_instance, request_factory, valid_api_key):
    ...
    def test_authenticate_with_extra_spaces_in_header(self, auth_instance, request_factory, valid_api_key):
    ...
    def test_authenticate_with_multiple_spaces_in_header(self, auth_instance, request_factory, valid_api_key):
    ...
    def test_authenticate_credentials_with_future_expiry(self, auth_instance, request_factory, tenant):
    ...
    def test_authenticate_credentials_returns_tenant_context(self, auth_instance, request_factory, valid_api_key):
