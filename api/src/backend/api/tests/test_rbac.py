...
class TestLimitedVisibility:
    ...
    def test_overviews_provider_details(
        self,
        authenticated_client_rbac_limited,
        providers_fixture,
    ):
        ...
        data = response.json()["data"]
        assert len(data) == 1
        assert data[0]["id"] == str(providers_fixture[0].id)
        assert data[0]["attributes"]["provider_type"] == providers_fixture[0].provider
    ...
    def test_overviews_provider_details_without_manage_providers(
        self,
        authenticated_client_no_permissions_rbac,
    ):
        response = authenticated_client_no_permissions_rbac.get(
