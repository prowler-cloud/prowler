...
class TestAPIKeyIntegrationWorkflows:
    ...
    def test_complete_api_key_lifecycle(self, authenticated_jwt_client, tenant):
        ...
        print(f"Created API key: {raw_api_key[:10]}...")  # Log partial key for debugging
    ...
    def test_api_key_activity_logging_integration(self, authenticated_jwt_client, tenant):
    ...
    def test_api_key_error_handling_integration(self, authenticated_jwt_client, tenant):
        ...
        nonexistent_client.defaults["HTTP_AUTHORIZATION"] = "ApiKey pk_nonexist.abcdef123456789012345678901234"
    ...
    def test_api_key_concurrent_usage(self, authenticated_jwt_client, tenant):
        ...
        for user, auth_info in results:
            assert user.is_anonymous  # API keys return AnonymousUser  
            assert auth_info["tenant_id"] == str(tenant.id)  # Same tenant-bound context!
    ...
    def test_api_key_permission_integration(self, authenticated_jwt_client, tenant):
        ...
        user, auth_info = auth.authenticate(request)
        assert user.is_anonymous  # API keys return AnonymousUser
        assert auth_info["tenant_id"] == str(tenant.id)  # Correct tenant context for permissions!
    ...
    def test_api_key_filtering_and_pagination_integration(self, authenticated_jwt_client, tenant):
    ...
    def test_api_key_creation_with_json_api_validation(self, authenticated_jwt_client, tenant):
