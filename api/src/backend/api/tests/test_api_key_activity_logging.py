...
class APIKeyActivityLoggingTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create(
            email="test@test.com", name="Test User", company_name="Test Company"
        )
    ...
    def test_middleware_logs_query_parameters_for_audit(self):
        ...
        request = self.factory.get(
            "/api/v1/scans?provider=aws&region=us-east-1&limit=100"
        )
        ...
        with patch("api.middleware.extract_auth_info") as mock_extract_auth_info:
            ...
            activity = APIKeyActivity.objects.first()
            expected_params = {"provider": "aws", "region": "us-east-1", "limit": "100"}
    ...
    def test_api_key_activity_model_indexes_for_security_queries(self):
        ...
        key_activities = APIKeyActivity.objects.filter(api_key=self.api_key).order_by(
            "-timestamp"
        )
        ...
        tenant_activities = APIKeyActivity.objects.filter(
            tenant_id=self.tenant.id
        ).order_by("-timestamp")
        ...
        incident_activities = APIKeyActivity.objects.filter(
            tenant_id=self.tenant.id, api_key=self.api_key, source_ip="192.168.1.2"
        ).order_by("-timestamp")
