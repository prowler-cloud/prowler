...
@pytest.mark.django_db
class TestSecurityHubRegionsView:
    """Test cases for SecurityHub regions endpoint."""

    @patch("api.v1.views.SecurityHub")
    @patch("api.v1.views.initialize_prowler_provider")
    def test_active_security_hub_regions_success(
        self,
        mock_initialize_provider,
        mock_security_hub_class,
        authenticated_client,
        providers_fixture,
    ):
        """Test successful retrieval of active SecurityHub regions."""
        # Get an AWS provider from fixtures
        aws_provider = None
        ...
        assert aws_provider is not None, "No AWS provider found in fixtures"
        ...
        # Mock SecurityHub with enabled regions
        mock_security_hub = MagicMock()
        mock_security_hub._enabled_regions = {
            "us-east-1": MagicMock(),
            "us-west-2": MagicMock(),
            "eu-west-1": MagicMock(),
        }
        ...
        # Make the request
        url = reverse("securityhub-regions", kwargs={"provider_id": aws_provider.id})
        response = authenticated_client.get(url)
        ...
        assert "regions" in response_data["data"]["attributes"]

        regions = response_data["data"]["attributes"]["regions"]
        ...
        mock_initialize_provider.assert_called_once_with(aws_provider)
    ...
    @patch("api.v1.views.SecurityHub")
    @patch("api.v1.views.initialize_prowler_provider")
    def test_active_security_hub_regions_no_audited_regions(
        self,
        mock_initialize_provider,
        mock_security_hub_class,
        authenticated_client,
        ...
        """Test SecurityHub regions when no audited regions are specified."""
        # Get an AWS provider from fixtures
        aws_provider = None
        for provider in providers_fixture:
            if provider.provider == Provider.ProviderChoices.AWS:
        ...
        # Verify SecurityHub was initialized with available regions (not audited ones)
        mock_security_hub_class.assert_called_once_with(
            aws_account_id="123456789012",
            aws_partition="aws",
            aws_session=mock_prowler_provider.session.current_session,
    ...
    def test_active_security_hub_regions_non_aws_provider(
        self, authenticated_client, providers_fixture
    ):
        """Test SecurityHub regions endpoint with non-AWS provider returns error."""
        # Get a non-AWS provider from fixtures
        non_aws_provider = None
        for provider in providers_fixture:
            if provider.provider != Provider.ProviderChoices.AWS:
                non_aws_provider = provider
                break

        if non_aws_provider is None:
            # Create a GCP provider for this test
            non_aws_provider = Provider.objects.create(
                alias="test-gcp",
                provider=Provider.ProviderChoices.GCP,
                tenant_id=authenticated_client.tenant.id,
            )

        # Make the request
        url = reverse(
            "securityhub-regions", kwargs={"provider_id": non_aws_provider.id}
        )
        response = authenticated_client.get(url)
        ...
        assert "detail" in response_data["errors"]
        assert "AWS type" in response_data["errors"]["detail"]

    def test_active_security_hub_regions_provider_not_found(self, authenticated_client):
        """Test SecurityHub regions endpoint with non-existent provider returns error."""
        non_existent_id = "f498b103-c760-4785-9a3e-e23fafbb7b02"

        # Make the request
        url = reverse("securityhub-regions", kwargs={"provider_id": non_existent_id})
        response = authenticated_client.get(url)

        # Assertions
        # The general exception handler catches get_object_or_404 and returns 400 with error message
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        response_data = response.json()
        assert "errors" in response_data
    ...
    @patch("api.v1.views.initialize_prowler_provider")
    def test_active_security_hub_regions_provider_initialization_error(
        self, mock_initialize_provider, authenticated_client, providers_fixture
    ):
        """Test SecurityHub regions endpoint when provider initialization fails."""
        # Get an AWS provider from fixtures
        aws_provider = None
        for provider in providers_fixture:
            if provider.provider == Provider.ProviderChoices.AWS:
        ...
        mock_initialize_provider.side_effect = Exception(
    ...
    @patch("api.v1.views.SecurityHub")
    @patch("api.v1.views.initialize_prowler_provider")
    def test_active_security_hub_regions_security_hub_error(
        self,
        mock_initialize_provider,
        mock_security_hub_class,
        authenticated_client,
        ...
        """Test SecurityHub regions endpoint when SecurityHub initialization fails."""
        # Get an AWS provider from fixtures
        aws_provider = None
        for provider in providers_fixture:
            if provider.provider == Provider.ProviderChoices.AWS:
        ...
        mock_prowler_provider.identity.account = "123456789012"
        mock_prowler_provider.identity.partition = "aws"
        mock_prowler_provider.identity.audited_regions = ["us-east-1"]
        mock_prowler_provider.session.current_session = MagicMock()
        mock_initialize_provider.return_value = mock_prowler_provider

        # Mock SecurityHub to raise an exception
        mock_security_hub_class.side_effect = Exception(
            "SecurityHub not enabled in region"
        )
    ...
    def test_active_security_hub_regions_unauthenticated(
        ...
        """Test SecurityHub regions endpoint requires authentication."""
        # Get an AWS provider from fixtures
        aws_provider = None
        for provider in providers_fixture:
            if provider.provider == Provider.ProviderChoices.AWS:
        ...
        if aws_provider is None:
            # Skip test if no AWS provider available
            pytest.skip("No AWS provider available in fixtures")
    ...
    @patch("api.v1.views.SecurityHub")
    @patch("api.v1.views.initialize_prowler_provider")
    def test_active_security_hub_regions_empty_enabled_regions(
        self,
        mock_initialize_provider,
        mock_security_hub_class,
        authenticated_client,
        ...
        """Test SecurityHub regions endpoint when no regions are enabled."""
        # Get an AWS provider from fixtures
        aws_provider = None
        for provider in providers_fixture:
            if provider.provider == Provider.ProviderChoices.AWS:
        ...
        # Mock SecurityHub with no enabled regions
        mock_security_hub = MagicMock()
        mock_security_hub._enabled_regions = {}
        mock_security_hub_class.return_value = mock_security_hub
