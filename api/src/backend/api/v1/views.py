...
@extend_schema_view(
    list=extend_schema(
        ...
        description="Retrieve a list of all configured providers with options for filtering by various criteria.",
        parameters=[
            OpenApiParameter(
                name="filter[alias]",
                type=str,
            ...
            OpenApiParameter(
                ...
                type=str,
                location=OpenApiParameter.QUERY,
                description='Filter by multiple provider aliases. Provide a comma-separated list, e.g. "aws_alias_1,azure_alias_2".',
                required=False,
            ),
            OpenApiParameter(
                ...
                type=bool,
                location=OpenApiParameter.QUERY,
                description="Filter by connection status. Set to True to return only connected providers, or False to return only providers with failed connections. If not specified, both connected and failed providers are included. Providers with no connection attempt (status is None) are excluded from this filter.",
                required=False,
            ),
            OpenApiParameter(
                name="filter[id]",
                type=OpenApiTypes.UUID,
                location=OpenApiParameter.QUERY,
                description="Filter by exact Prowler provider ID (UUIDv4).",
                required=False,
            ),
            OpenApiParameter(
                name="filter[id__in]",
                type=str,
                location=OpenApiParameter.QUERY,
                description='Filter by multiple Prowler provider IDs. Provide a comma-separated list of UUIDs, e. g.: "a1b2c3d4-5678-90ab-cdef-1234567890ab,deadbeef-1234-5678-9abc-def012345678,0f1e2d3c-4b5a-6978-8c9d-0e1f2a3b4c5d".',
                required=False,
            ),
            OpenApiParameter(
            ...
            OpenApiParameter(
                ...
                type=str,
                location=OpenApiParameter.QUERY,
                description='Search a term across "provider", "alias" and "uid"',
                required=False,
            ),
            OpenApiParameter(
                name="filter[uid]",
                type=str,
                location=OpenApiParameter.QUERY,
                description="Filter by exact provider ID (AWS account ID, Azure subscription ID, GCP project ID, etc.)",
                required=False,
            ),
            OpenApiParameter(
                ...
                type=str,
                location=OpenApiParameter.QUERY,
                description="Filter by partial provider ID match (AWS account ID, Azure subscription ID, GCP project ID, etc.)",
                required=False,
            ),
            OpenApiParameter(
                name="filter[uid__in]",
                type=str,
                location=OpenApiParameter.QUERY,
                description='Filter by multiple provider IDs (AWS account ID, Azure subscription ID, GCP project ID, etc.). For example: "1234567890,a1b2c3d4-5678-90ab-cdef-1234567890ab"',
                required=False,
            ),
            OpenApiParameter(
                ...
                type=OpenApiTypes.DATE,
                location=OpenApiParameter.QUERY,
                description="Filter by exact date when provider was lastly updated (format: YYYY-MM-DD).",
                required=False,
            ),
            OpenApiParameter(
            ...
            OpenApiParameter(
                ...
                type=str,
                location=OpenApiParameter.QUERY,
                description="Sort the results by the specified fields. Use '-' prefix for descending order. For example: \"-provider,inserted_at\", this first sorts by provider alphabetically and then inside of each category by inserted_at date",
                required=False,
            ),
        ],
        filters=True,
    ...
    retrieve=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                ...
                type=OpenApiTypes.UUID,
                location=OpenApiParameter.PATH,
                description="Prowler provider ID (UUIDv4 generated by Prowler)",
                required=True,
            ),
            OpenApiParameter(
            ...
            OpenApiParameter(
                ...
                type=str,
                location=OpenApiParameter.QUERY,
                description="Include related resources in the response, accepts a string as a comma-separated list",
                required=False,
            ),
        ],
    ),
    create=extend_schema(
        ...
        description="Create a new provider in the current Prowler Tenant. The provider represents a cloud account or environment that Prowler will scan for security findings.",
        request=ProviderCreateSerializer,
        responses={201: OpenApiResponse(response=ProviderSerializer)},
    ),
    partial_update=extend_schema(
        tags=["Provider"],
        summary="Update provider details",
        description="Update some fields of a specific provider",
        parameters=[
            OpenApiParameter(
                name="id",
                type=OpenApiTypes.UUID,
...
class ProviderViewSet(BaseRLSViewSet):
    ...
    @extend_schema(
        ...
        description="Verify that a configured provider can successfully establish a connection to the actual provider endpoints.",
        request=None,
        parameters=[
            OpenApiParameter(
                name="id",
...
@extend_schema_view(
    ...
    retrieve=extend_schema(
        tags=["Provider"],
        summary="Get provider secret details",
    ...
    create=extend_schema(
        tags=["Provider"],
        summary="Add provider secret",
        description="Add or update complete credentials for an existing provider",
        request=ProviderSecretCreateSerializer,
        responses={201: OpenApiResponse(response=ProviderSecretSerializer)},
    ),
    partial_update=extend_schema(
        tags=["Provider"],
        summary="Update provider secret",
        ...
        request=ProviderSecretUpdateSerializer,
        responses={200: OpenApiResponse(response=ProviderSecretSerializer)},
    ),
    destroy=extend_schema(
        tags=["Provider"],
        summary="Delete provider secret",
        ...
        responses={204: OpenApiResponse(description="Secret deleted successfully")},
    ),
)
@method_decorator(CACHE_DECORATOR, name="list")
@method_decorator(CACHE_DECORATOR, name="retrieve")
