        ...
        for instance in ecs_client.instances.values():
            ...
            if instance.security_groups:
                ...
                report.status_extended = (
                    f"ECS instance {instance.name} ({instance.id}) has security group(s) attached: {sg_names}."
                )
            else:
                report.status = "FAIL"
                report.status_extended = (
                    f"ECS instance {instance.name} ({instance.id}) does not have any security groups attached."
                )
