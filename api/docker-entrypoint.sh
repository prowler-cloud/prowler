...
start_worker() {
  ...
  poetry run python -m celery -A config.celery worker -l "${DJANGO_LOGGING_LEVEL:-info}" -Q celery,scans,scan-reports,deletion,backfill,overview,integrations,compliance,scan-daily-severity -E --max-tasks-per-child 1
}
