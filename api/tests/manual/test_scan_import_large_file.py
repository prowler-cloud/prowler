...
AWS_SERVICES = [
    ...
    ("acm", ["acm_certificates_expiration_check", "acm_certificates_transparency_logs_enabled"]),
    ("apigateway", ["apigateway_restapi_logging_enabled", "apigateway_restapi_authorizers_enabled"]),
    ("cloudfront", ["cloudfront_distributions_https_enabled", "cloudfront_distributions_logging_enabled"]),
    ("cloudtrail", ["cloudtrail_multi_region_enabled", "cloudtrail_log_file_validation_enabled"]),
    ("cloudwatch", ["cloudwatch_log_group_retention_policy_specific_days_enabled"]),
    ("config", ["config_recorder_all_regions_enabled"]),
    ("dynamodb", ["dynamodb_tables_pitr_enabled", "dynamodb_tables_kms_cmk_encryption_enabled"]),
    ("ec2", ["ec2_instance_imdsv2_enabled", "ec2_instance_public_ip", "ec2_securitygroup_default_restrict_traffic"]),
    ("ecr", ["ecr_repositories_scan_images_on_push_enabled", "ecr_repositories_lifecycle_policy_enabled"]),
    ...
    ("iam", ["iam_root_hardware_mfa_enabled", "iam_user_mfa_enabled_console_access", "iam_password_policy_minimum_length_14"]),
    ("kms", ["kms_cmk_rotation_enabled"]),
    ("lambda", ["awslambda_function_url_public", "awslambda_function_using_supported_runtimes"]),
    ("rds", ["rds_instance_storage_encrypted", "rds_instance_multi_az", "rds_instance_backup_enabled"]),
    ("redshift", ["redshift_cluster_audit_logging"]),
    ("s3", ["s3_bucket_public_access_block_enabled", "s3_bucket_default_encryption", "s3_bucket_versioning_enabled"]),
...
def main():
    ...
    parser.add_argument(
        "--ocsf-only",
        action="store_true",
        help="Only test OCSF/JSON format"
    )
    parser.add_argument(
        "--csv-only",
        action="store_true",
        help="Only test CSV format"
    )
