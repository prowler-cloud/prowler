...
- Label Postgres connections with `application_name="<component>:<alias>"` (component injected per process via `DJANGO_APP_COMPONENT`) so connections are attributable in `pg_stat_activity` and Performance Insights' "Application" dimension [(#11494)](https://github.com/prowler-cloud/prowler/pull/11494)
