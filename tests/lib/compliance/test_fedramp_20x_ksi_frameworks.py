...
class TestFedRAMP20xKSIFrameworks:
    ...
    def test_fedramp_20x_ksi_aws_framework_exists(self, compliance_path):
        ...
        assert aws_ksi_path.exists(), f"FedRAMP 20x KSI AWS framework not found at {aws_ksi_path}"
    ...
    def test_fedramp_20x_ksi_azure_framework_exists(self, compliance_path):
        ...
        assert azure_ksi_path.exists(), f"FedRAMP 20x KSI Azure framework not found at {azure_ksi_path}"
    ...
    def test_fedramp_20x_ksi_gcp_framework_exists(self, compliance_path):
        ...
        assert gcp_ksi_path.exists(), f"FedRAMP 20x KSI GCP framework not found at {gcp_ksi_path}"
    ...
    def test_fedramp_20x_ksi_aws_has_all_ksi_requirements(self, compliance_path):
        ...
        with open(aws_ksi_path) as f:
            ...
            expected_ksis = ["ksi-ced", "ksi-cmt", "ksi-cna", "ksi-iam", "ksi-inr", 
                           "ksi-mla", "ksi-piy", "ksi-rpl", "ksi-svc", "ksi-tpr"]
    ...
    def test_fedramp_20x_ksi_aws_requirements_have_checks(self, compliance_path):
        ...
        with open(aws_ksi_path) as f:
            ...
            for req in data["Requirements"]:
                ...
                assert len(req["Checks"]) > 0, f"Requirement {req['Id']} has empty Checks"
                assert "Attributes" in req, f"Requirement {req['Id']} missing Attributes"
    ...
    def test_fedramp_20x_ksi_frameworks_version_consistency(self, compliance_path):
        ...
        assert aws_data["Version"] == azure_data["Version"] == gcp_data["Version"], \
    ...
    def test_fedramp_20x_ksi_nist_controls_mapping(self, compliance_path):
        ...
        for provider, path_suffix in [("aws", "aws/fedramp_20x_ksi_aws.json"),
            ...
            with open(framework_path) as f:
                data = json.load(f)
                for req in data["Requirements"]:
                    assert len(req["Attributes"]) > 0, f"{provider}: Requirement {req['Id']} has no attributes"
                    for attr in req["Attributes"]:
                        assert "NistControls" in attr, f"{provider}: Requirement {req['Id']} attribute missing NistControls"
                        assert attr["NistControls"], f"{provider}: Requirement {req['Id']} has empty NistControls"