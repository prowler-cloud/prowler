...
class TestFedRAMP20xKSIGCP:
    def test_get_table_with_ksi_data(self):
        test_data = pd.DataFrame({
            ...
            "REQUIREMENTS_ATTRIBUTES_SECTION": ["Cybersecurity Education", "Change Management", "Identity and Access Management"],
        ...
        for section in result:
            if "tables" in section:
                for table in section["tables"]:
                    ...
                    assert "Cybersecurity Education" in df["REQUIREMENTS_DESCRIPTION"].values
                    assert "Change Management" in df["REQUIREMENTS_DESCRIPTION"].values
                    assert "Identity and Access Management" in df["REQUIREMENTS_DESCRIPTION"].values
    ...
    def test_get_table_gcp_specific_locations(self):
        test_data = pd.DataFrame({
            ...
            "REQUIREMENTS_ATTRIBUTES_SECTION": ["Service Configuration", "Third-Party Information Resources"],
        ...
        for section in result:
            if "tables" in section:
                for table in section["tables"]:
                    df = table["data"]
                    assert "Service Configuration" in df["REQUIREMENTS_DESCRIPTION"].values
                    assert "Third-Party Information Resources" in df["REQUIREMENTS_DESCRIPTION"].values
                    # Check GCP locations are preserved
                    assert "asia-southeast1" in df["REGION"].values or "us-west1" in df["REGION"].values
    ...
    def test_get_table_monitoring_logging_auditing(self):
        test_data = pd.DataFrame({
            ...
            "REQUIREMENTS_ATTRIBUTES_SECTION": ["Monitoring, Logging, and Auditing", "Monitoring, Logging, and Auditing", "Monitoring, Logging, and Auditing"],
        ...
        for section in result:
            if "tables" in section:
                for table in section["tables"]:
                    df = table["data"]
                    assert "Monitoring, Logging, and Auditing" in df["REQUIREMENTS_DESCRIPTION"].values
    ...
    def test_get_table_policy_and_inventory(self):
        test_data = pd.DataFrame({
            ...
            "REQUIREMENTS_ATTRIBUTES_SECTION": ["Policy and Inventory", "Policy and Inventory"],
        ...
        for section in result:
            if "tables" in section:
                for table in section["tables"]:
                    df = table["data"]
                    assert "Policy and Inventory" in df["REQUIREMENTS_DESCRIPTION"].values
