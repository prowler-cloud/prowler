...
class TestFedRAMP20xKSIAzure:
        ...
        for section in result:
            if "tables" in section:
                for table in section["tables"]:
                    ...
                    assert "northeurope" in df["REGION"].values or "southeastasia" in df["REGION"].values
    ...
    def test_get_table_cloud_native_architecture(self):
        test_data = pd.DataFrame({
            ...
            "REQUIREMENTS_ATTRIBUTES_SECTION": ["Cloud Native Architecture", "Cloud Native Architecture", "Cloud Native Architecture"],
        ...
        for section in result:
            if "tables" in section:
                for table in section["tables"]:
                    df = table["data"]
                    assert "Cloud Native Architecture" in df["REQUIREMENTS_DESCRIPTION"].values
                    # Verify all checks are included
                    assert len(df[df["CHECKID"].isin(["network_check", "storage_check", "compute_check"])]) >= 3
