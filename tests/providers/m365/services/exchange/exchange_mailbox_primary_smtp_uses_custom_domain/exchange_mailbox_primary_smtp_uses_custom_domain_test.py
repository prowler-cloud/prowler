...
class Test_exchange_mailbox_primary_smtp_uses_custom_domain:
    ...
    def test_no_mailboxes(self):
        """MANUAL: No mailboxes retrieved. PowerShell unavailable."""
    ...
    def test_custom_domain_passes(self):
        """PASS: Mailbox primary SMTP uses a verified custom domain."""
        exchange_client = mock.MagicMock()
        exchange_client.audited_tenant = "audited_tenant"
        exchange_client.audited_domain = DOMAIN
        ...
        with (
            ...
            assert "verified custom domain" in result[0].status_extended
    ...
    def test_mixed_mailboxes(self):
        ...
        with (
            ...
            assert (
                ...
                == "Mailbox user1@contoso.com (UserMailbox) has primary SMTP address user1@contoso.com using a verified custom domain."
            )
            assert result[1].status == "FAIL"
            assert (
                result[1].status_extended
                == "Mailbox shared@contoso.onmicrosoft.com (SharedMailbox) has primary SMTP address shared@contoso.onmicrosoft.com using the .onmicrosoft.com domain instead of a verified custom domain."
            )

    def test_room_mailbox_custom_domain(self):
        """PASS: Room mailbox using a verified custom domain."""
        exchange_client = mock.MagicMock()
        exchange_client.audited_tenant = "audited_tenant"
        exchange_client.audited_domain = DOMAIN
