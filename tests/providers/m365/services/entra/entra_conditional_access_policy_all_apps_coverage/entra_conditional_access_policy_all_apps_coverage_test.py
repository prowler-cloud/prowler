...
class Test_entra_conditional_access_policy_all_apps_coverage:
    def test_no_conditional_access_policies(self):
        ...
        with (
            ...
            mock.patch(
                "prowler.providers.m365.services.entra.entra_conditional_access_policy_all_apps_coverage.entra_conditional_access_policy_all_apps_coverage.entra_client",
                new=entra_client,
            ),
        ):
            from prowler.providers.m365.services.entra.entra_conditional_access_policy_all_apps_coverage.entra_conditional_access_policy_all_apps_coverage import (
                entra_conditional_access_policy_all_apps_coverage,
            )

            entra_client.conditional_access_policies = {}

            check = entra_conditional_access_policy_all_apps_coverage()
    ...
    def test_multiple_policies_one_valid(self):
        """
        Multiple policies where one is valid:
         - First policy is disabled
         - Second policy targets specific apps
         - Third policy is valid (enabled, targets all apps)

         Expected PASS with the valid policy.
        """
        ...
        with (
            ...
            assert len(result) == 1
            assert result[0].status == "PASS"
            expected_status_extended = f"Conditional Access Policy '{valid_display_name}' targets all cloud apps."
            assert result[0].status_extended == expected_status_extended
            assert result[0].resource_name == valid_display_name
            assert result[0].resource_id == valid_policy_id
