    ...
    def test_zone_unreadable_is_manual(self):
        ...
        zone_client.zones = {
            ZONE_ID: CloudflareZone(
                id=ZONE_ID,
                name=ZONE_NAME,
                read_errors={"browser_check": "PermissionDeniedError"},
            )
        }
        ...
        with (
            ...
            assert (
                ...
                == f"Cannot evaluate the Browser Integrity Check setting for zone {ZONE_NAME}: the API token is missing the Zone Settings Read permission."
            )
