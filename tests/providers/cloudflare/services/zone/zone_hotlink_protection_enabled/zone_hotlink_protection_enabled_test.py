    ...
    def test_zone_unreadable_is_manual(self):
        ...
        zone_client.zones = {
            ZONE_ID: CloudflareZone(
                id=ZONE_ID,
                name=ZONE_NAME,
                read_errors={"hotlink_protection": "PermissionDeniedError"},
            )
        }
        ...
        with (
            ...
            assert (
                ...
                == f"Cannot evaluate the Hotlink Protection setting for zone {ZONE_NAME}: the API token is missing the Zone Settings Read permission."
            )
