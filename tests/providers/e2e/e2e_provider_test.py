...
from prowler.providers.e2e.exceptions.exceptions import E2eCredentialsError


class TestE2eProvider:
    @patch("prowler.providers.e2e.e2e_provider.load_and_validate_config_file")
    def test_e2e_provider_init_success(self, mock_load_config):
    ...
    @patch("prowler.providers.e2e.e2e_provider.load_and_validate_config_file")
    def test_e2e_provider_init_missing_credentials(self, mock_load_config):
        mock_load_config.return_value = {}

        with pytest.raises(E2eCredentialsError):
            E2eProvider(api_key="", auth_token="token", project_id="1")
    ...
    @patch("prowler.providers.e2e.e2e_provider.load_and_validate_config_file")
    def test_e2e_provider_init_from_env(self, mock_load_config):
        mock_load_config.return_value = {}
    ...
    @patch("prowler.providers.e2e.e2e_provider.E2eProvider.setup_session")
    def test_e2e_test_connection_success(self, mock_setup_session):
        ...
        result = E2eProvider.test_connection(
            api_key="key",
            auth_token="token",
            project_id=1,
            locations=["Delhi"],
    ...
    def test_e2e_test_connection_missing_credentials(self):
        with pytest.raises(E2eCredentialsError):
            E2eProvider.test_connection(api_key="", auth_token="", project_id=None)
