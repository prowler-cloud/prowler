...
class Test_postgresql_flexible_server_log_checkpoints_on:
    ...
    def test_flexible_servers_log_checkpoints_off(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Flexible Postgresql server {postgresql_server_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has log_checkpoints disabled"
            )
    ...
    def test_flexible_servers_log_checkpoints_on(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Flexible Postgresql server {postgresql_server_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has log_checkpoints enabled"
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == postgresql_server_name
            assert result[0].resource_id == postgresql_server_id
