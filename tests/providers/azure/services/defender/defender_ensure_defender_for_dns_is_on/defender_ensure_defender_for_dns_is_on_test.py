...
class Test_defender_ensure_defender_for_dns_is_on:
    ...
    def test_defender_dns_pricing_tier_not_standard(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Defender plan Defender for DNS from subscription {AZURE_SUBSCRIPTION_DISPLAY} is set to OFF (pricing tier not standard)."
            )
    ...
    def test_defender_dns_pricing_tier_standard(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Defender plan Defender for DNS from subscription {AZURE_SUBSCRIPTION_DISPLAY} is set to ON (pricing tier standard)."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == "Defender plan DNS"
            assert result[0].resource_id == resource_id
