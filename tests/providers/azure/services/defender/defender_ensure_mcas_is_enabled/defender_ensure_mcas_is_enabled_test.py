...
class Test_defender_ensure_mcas_is_enabled:
    ...
    def test_defender_mcas_enabled(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"Microsoft Defender for Cloud Apps is enabled for subscription {AZURE_SUSCRIPTION}."
            )
            assert result[0].subscription == AZURE_SUSCRIPTION
            assert result[0].resource_name == "MCAS"
            assert result[0].resource_id == resource_id
    ...
    def test_defender_mcas_no_settings(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"Microsoft Defender for Cloud Apps not exists for subscription {AZURE_SUSCRIPTION}."
            )
            assert result[0].subscription == AZURE_SUSCRIPTION
            assert result[0].resource_name == "MCAS"
