...
class Test_aks_clusters_public_access_disabled:
    ...
    def test_aks_cluster_public_fqdn(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Public access to nodes is enabled for cluster 'cluster_name' in subscription '{AZURE_SUBSCRIPTION_DISPLAY}'"
            )
    ...
    def test_aks_cluster_private_fqdn(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Public access to nodes is disabled for cluster 'cluster_name' in subscription '{AZURE_SUBSCRIPTION_DISPLAY}'"
            )
            assert result[0].resource_id == cluster_id
            assert result[0].resource_name == "cluster_name"
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
