...
class Test_mysql_flexible_server_high_availability_enabled:
    ...
    def test_mysql_high_availability_disabled(self):
        server_id = str(uuid4())
        ...
        with (
            ...
            assert (
                ...
                == f"High availability is disabled for server {server_name} in subscription {AZURE_SUBSCRIPTION_DISPLAY}."
            )
    ...
    def test_mysql_high_availability_not_set(self):
        server_id = str(uuid4())
        server_name = "test-server"
        mysql_client = mock.MagicMock
        mysql_client.subscriptions = {AZURE_SUBSCRIPTION_ID: AZURE_SUBSCRIPTION_NAME}
    ...
    def test_mysql_high_availability_enabled(self):
        server_id = str(uuid4())
        server_name = "test-server"
        mysql_client = mock.MagicMock
        mysql_client.subscriptions = {AZURE_SUBSCRIPTION_ID: AZURE_SUBSCRIPTION_NAME}
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"High availability is enabled for server {server_name} in subscription {AZURE_SUBSCRIPTION_DISPLAY}."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == server_name
            assert result[0].resource_id == server_id
