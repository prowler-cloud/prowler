...
class Test_storage_default_network_access_rule_is_denied:
    ...
    def test_storage_storage_accounts_default_network_access_rule_allowed(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has network access rule set to Allow."
            )
    ...
    def test_storage_storage_accounts_default_network_access_rule_denied(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has network access rule set to Deny."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
