...
class Test_storage_secure_transfer_required_is_enabled:
    ...
    def test_storage_storage_accounts_secure_transfer_required_disabled(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has secure transfer required disabled."
            )
    ...
    def test_storage_storage_accounts_secure_transfer_required_enabled(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has secure transfer required enabled."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
