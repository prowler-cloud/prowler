...
class Test_storage_ensure_encryption_with_customer_managed_keys:
    ...
    def test_storage_storage_accounts_encryption_without_customer_managed_keys(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} does not encrypt with CMKs."
            )
    ...
    def test_storage_storage_accounts_encryption_with_customer_managed_keys(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} encrypts with CMKs."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
