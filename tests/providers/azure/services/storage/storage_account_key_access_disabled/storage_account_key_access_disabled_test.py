...
class Test_storage_account_key_access_disabled:
    ...
    def test_storage_account_shared_key_access_enabled(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has shared key access enabled."
            )
    ...
    def test_storage_account_shared_key_access_disabled(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has shared key access disabled."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
