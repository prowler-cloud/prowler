...
class Test_storage_key_rotation_90_dayss:
    ...
    def test_storage_storage_key_rotation_91_days(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has an invalid key expiration period of {expiration_days} days."
            )
    ...
    def test_storage_storage_key_rotation_90_days(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has a key expiration period of {expiration_days} days."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
    ...
    def test_storage_storage_no_key_rotation(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has no key expiration period set."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
