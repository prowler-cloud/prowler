...
class Test_storage_ensure_minimum_tls_version_12:
    ...
    def test_storage_storage_accounts_tls_not_1_2(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} does not have TLS version set to 1.2."
            )
    ...
    def test_storage_storage_accounts_tls_1_2(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has TLS version set to 1.2."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
