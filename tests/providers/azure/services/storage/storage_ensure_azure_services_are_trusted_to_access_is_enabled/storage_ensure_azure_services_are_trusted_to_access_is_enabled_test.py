...
class Test_storage_ensure_azure_services_are_trusted_to_access_is_enabled:
    ...
    def test_storage_storage_accounts_azure_services_are_not_trusted_to_access(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} does not allow trusted Microsoft services to access this storage account."
            )
    ...
    def test_storage_storage_accounts_azure_services_are_trusted_to_access(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} allows trusted Microsoft services to access this storage account."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
