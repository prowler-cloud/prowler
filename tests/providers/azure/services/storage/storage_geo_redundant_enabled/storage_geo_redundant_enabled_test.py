...
class Test_storage_geo_redundant_enabled:
    ...
    def test_storage_account_standard_grs_enabled(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has Geo-redundant storage {replication_setting} enabled."
            )
    ...
    def test_storage_account_standard_lrs_disabled(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Storage account {storage_account_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} does not have Geo-redundant storage enabled, it has {replication_setting} instead."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == storage_account_name
            assert result[0].resource_id == storage_account_id
