...
class Test_sqlserver_azuread_administrator_enabled:
    ...
    def test_sql_servers_azuread_no_administrator(self):
        ...
        with (
            ...
            assert (
                ...
                == f"SQL Server {sql_server_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} does not have an Active Directory administrator."
            )
    ...
    def test_sql_servers_azuread_administrator_active_directory(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"SQL Server {sql_server_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has an Active Directory administrator."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == sql_server_name
            assert result[0].resource_id == sql_server_id
