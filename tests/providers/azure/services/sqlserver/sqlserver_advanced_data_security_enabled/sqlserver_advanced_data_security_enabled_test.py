...
class Test_sqlserver_advanced_data_security_enabled:
    def test_no_sql_servers(self):
        ...
        with (
            ...
            mock.patch(
                "prowler.providers.azure.services.sqlserver.sqlserver_advanced_data_security_enabled.sqlserver_advanced_data_security_enabled.sqlserver_client",
                new=sqlserver_client,
            ),
        ):
            from prowler.providers.azure.services.sqlserver.sqlserver_advanced_data_security_enabled.sqlserver_advanced_data_security_enabled import (
    ...
    def test_sql_servers_ads_enabled(self):
        sqlserver_client = mock.MagicMock
        sqlserver_client.subscriptions = {
            AZURE_SUBSCRIPTION_ID: AZURE_SUBSCRIPTION_NAME
        }
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"SQL Server {sql_server_name} from subscription {AZURE_SUBSCRIPTION_DISPLAY} has Advanced Data Security enabled."
            )
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
            assert result[0].resource_name == sql_server_name
            assert result[0].resource_id == sql_server_id
