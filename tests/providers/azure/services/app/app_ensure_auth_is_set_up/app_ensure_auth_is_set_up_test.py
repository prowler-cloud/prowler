...
class Test_app_ensure_auth_is_set_up:
    ...
    def test_app_auth_enabled(self):
        ...
        with (
            ...
            assert (
                ...
                == f"Authentication is set up for app 'app_id-1' in subscription '{AZURE_SUBSCRIPTION_DISPLAY}'."
            )
    ...
    def test_app_auth_disabled(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Authentication is not set up for app 'app_id-1' in subscription '{AZURE_SUBSCRIPTION_DISPLAY}'."
            )
            assert result[0].resource_name == "app_id-1"
            assert result[0].resource_id == resource_id
            assert result[0].subscription == AZURE_SUBSCRIPTION_ID
