...
class Test_rds_instance_public_access:
    def test_no_db_instances(self):
        ...
        with mock.patch(
            ...
            "prowler.providers.alibabacloud.services.rds.rds_instance_public_access.rds_instance_public_access.rds_client",
            new=rds_client,
        ):
            from prowler.providers.alibabacloud.services.rds.rds_instance_public_access.rds_instance_public_access import (
                rds_instance_public_access,
            )

            check = rds_instance_public_access()
            result = check.execute()
            assert len(result) == 0

    def test_db_instance_public_access_pass(self):
        rds_client = mock.MagicMock
        ...
        with mock.patch(
            ...
            assert "does not allow public access" in result[0].status_extended

    def test_db_instance_public_access_fail(self):
        rds_client = mock.MagicMock
        ...
        with mock.patch(
            ...
            assert result[0].resource_id == db_instance_id
            assert "allows public access" in result[0].status_extended
