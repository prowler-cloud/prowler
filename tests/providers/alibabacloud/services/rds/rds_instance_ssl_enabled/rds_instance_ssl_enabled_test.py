...
class Test_rds_instance_ssl_enabled:
    ...
    def test_db_instance_ssl_enabled_pass(self):
        rds_client = mock.MagicMock
        ...
        with mock.patch(
            ...
            assert "has SSL enabled for encrypted connections" in result[0].status_extended

    def test_db_instance_ssl_enabled_fail(self):
        rds_client = mock.MagicMock
        ...
        with mock.patch(
            ...
            assert result[0].resource_id == db_instance_id
            assert "does not have SSL enabled. Enable SSL to encrypt data in transit" in result[0].status_extended
