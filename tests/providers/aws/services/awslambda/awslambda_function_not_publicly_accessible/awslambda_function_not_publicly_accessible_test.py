...
class Test_awslambda_function_not_publicly_accessible:
    ...
    def test_no_functions(self):
        ...
        with mock.patch(
            ...
            "prowler.providers.aws.services.awslambda.awslambda_function_not_publicly_accessible.awslambda_function_not_publicly_accessible.ec2_client",
    ...
    @mock_aws
    def test_function_public_but_not_internet_exposed(self):
        ...
        with mock.patch(
            ...
            assert (
                ...
                == f"Lambda function {function_name} is not publicly accessible."
            )
    ...
    def test_function_public_with_alb(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"Lambda function {function_name} is publicly accessible."
            )
            assert result[0].resource_tags == [{"tag1": "value1", "tag2": "value2"}]

    @mock_aws
    def test_function_linked_through_alb_but_not_public_due_to_restrictive_policies(
        self,
    ):
        # Create the mock VPC
        ec2_client = client("ec2", region_name=AWS_REGION_EU_WEST_1)
        ...
        # Set restrictive policy on Lambda
        lambda_client.put_function_concurrency(
            FunctionName=function_name,
            ReservedConcurrentExecutions=0,  # Restrictive policy: no concurrent executions allowed
        )
    ...
    @mock_aws
    def test_function_not_public_with_alb_for_secure_security_group(self):
        # Create the mock VPC
        ec2_client = client("ec2", region_name=AWS_REGION_EU_WEST_1)
        vpc = ec2_client.create_vpc(CidrBlock="10.0.0.0/16")
        vpc_id = vpc["Vpc"]["VpcId"]
            ...
            IpPermissions=[
                {
                    ...
                    "FromPort": 80,
                    "ToPort": 80,
                    "IpRanges": [{"CidrIp": "10.0.2.0/24"}],  # Private IP range
                }
            ],
        )
    ...
    # @mock_aws
    # def test_function_not_public_policies(self):
    #     # Create the mock IAM role
    #     iam_client = client("iam", region_name=AWS_REGION_EU_WEST_1)
    #     role_name = "test-role"
    #     assume_role_policy_document = {
