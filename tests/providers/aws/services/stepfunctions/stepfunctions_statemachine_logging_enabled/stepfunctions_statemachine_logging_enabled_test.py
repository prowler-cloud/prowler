...
class Test_stepfunctions_statemachine_logging_enabled:
    ...
    def test_state_machine_logging_level_mismatch(self):
        mocked_aws_provider = set_mocked_aws_provider([AWS_REGION_EU_WEST_1])
        stepfunctions_client = StepFunctions(mocked_aws_provider)
        stepfunctions_client.state_machines[STATE_MACHINE_ARN] = create_state_machine(
            "TestStateMachine",
        ...
        stepfunctions_client.audit_config = {"statemachines_log_level": "ALL"}
        ...
        with patch(
            ...
            assert (
                result[0].status_extended
                == "Step Functions state machine 'TestStateMachine' have the log level 'ERROR' which does not match the one specified in the configuration 'ALL'."
            )
            assert result[0].resource_id == STATE_MACHINE_ID
            assert result[0].resource_arn == STATE_MACHINE_ARN
            assert result[0].region == AWS_REGION_EU_WEST_1

    def test_invalid_log_level_configuration(self):
        mocked_aws_provider = set_mocked_aws_provider([AWS_REGION_EU_WEST_1])
        stepfunctions_client = StepFunctions(mocked_aws_provider)
        stepfunctions_client.state_machines[STATE_MACHINE_ARN] = create_state_machine(
            "TestStateMachine",
        ...
        stepfunctions_client.audit_config = {"statemachines_log_level": "INVALID_LEVEL"}
        ...
        with patch(
            ...
            assert (
                result[0].status_extended
                == "Invalid value found for statemachines_log_level parameter: INVALID_LEVEL."
            )
            assert result[0].resource_id == STATE_MACHINE_ID
            assert result[0].resource_arn == STATE_MACHINE_ARN
            assert result[0].region == AWS_REGION_EU_WEST_1
