...
class Test_kms_cmk_not_deleted_unintentionally:
    ...
    @mock_aws
    def test_kms_cmk_not_deleted_unintentionally(self):
        from prowler.providers.aws.services.kms.kms_service import KMS
        ...
        with mock.patch(
            ...
            assert (
                ...
                == f"KMS CMK {key['KeyId']} is not used and is not scheduled for deletion."
            )
    ...
    def test_kms_cmk_deleted_unintentionally(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"KMS CMK {key['KeyId']} is scheduled for deletion."
            )
            assert result[0].resource_id == key["KeyId"]
            assert result[0].resource_arn == key["Arn"]
