...
class Test_apigateway_endpoint_public_without_authorizer:
    ...
    @mock_apigateway
    def test_apigateway_one_private_rest_api_without_authorizer(self):
    ...
    @mock_apigateway
    def test_apigateway_one_private_rest_api_with_authorizer(self):
        # Create APIGateway Mocked Resources
        apigateway_client = client("apigateway", region_name=AWS_REGION)
        # Create APIGateway Deployment Stage
        rest_api = apigateway_client.create_rest_api(
    ...
    def test_apigateway_one_public_rest_api_without_authorizer(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"API Gateway test-rest-api ID {rest_api['id']} is internet accesible without an authorizer."
            )
            assert result[0].resource_id == "test-rest-api"
            assert (
                result[0].resource_arn
    ...
    def test_apigateway_one_public_rest_api_with_authorizer(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"API Gateway test-rest-api ID {rest_api['id']} is internet accesible with an authorizer."
            )
            assert result[0].resource_id == "test-rest-api"
            assert (
                result[0].resource_arn
