...
class Test_autoscaling_group_multiple_instances_types:
    ...
    def test_groups_with_multi_az_one_or_less_instances(self):
        ...
        with mock.patch(
            ...
            assert (
                ...
                == f"Autoscaling group {autoscaling_group_name} has one or less instance types."
            )
    ...
    @mock_aws
    def test_groups_with_multi_az_multi_instances(self):
        with mock.patch(
            ...
            with mock.patch(
                ...
                assert (
                    result[0].status_extended
                    == f"Autoscaling group {autoscaling_group_name} has multiple instance types in multiple availability zones."
                )
                assert result[0].resource_id == autoscaling_group_name
                assert result[0].resource_arn == autoscaling_group_arn
                assert result[0].region == AWS_REGION_US_EAST_1
    ...
    def test_groups_with_single_az_multi_instances(self):
        with mock.patch(
            ...
            with mock.patch(
                ...
                assert (
                    result[0].status_extended
                    == f"Autoscaling group {autoscaling_group_name} has multiple instance type in single availability zone."
                )
                assert result[0].resource_id == autoscaling_group_name
                assert result[0].resource_tags == []
                assert result[0].resource_arn == autoscaling_group_arn
