...
class Test_ELBv2_Service:
    ...
    def test_describe_target_groups(self):
        ...
        assert len(elbv2.target_groups) == 1
        assert target_group["TargetGroupArn"] in elbv2.target_groups
        assert (
            elbv2.target_groups[target_group["TargetGroupArn"]].name
            == "my-target-group"
        )
        assert elbv2.target_groups[target_group["TargetGroupArn"]].protocol == "HTTP"
        assert elbv2.target_groups[target_group["TargetGroupArn"]].port == 80
        assert elbv2.target_groups[target_group["TargetGroupArn"]].vpc_id == vpc.id
        assert elbv2.target_groups[
            target_group["TargetGroupArn"]
        ].load_balancer_arns == [lb["LoadBalancerArn"]]
        assert (
            elbv2.target_groups[target_group["TargetGroupArn"]].target_type
            == "instance"
        )
        assert elbv2.target_groups[target_group["TargetGroupArn"]].targets == {}
