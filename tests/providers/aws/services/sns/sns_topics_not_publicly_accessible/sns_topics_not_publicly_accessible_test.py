...
class Test_sns_topics_not_publicly_accessible:
    ...
    def test_topic_public_with_principal_organization(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"SNS topic {topic_name} is not public because its policy only allows access from the organization {org_id}."
            )
            assert result[0].resource_id == topic_name
            assert result[0].resource_arn == topic_arn
            assert result[0].region == AWS_REGION_EU_WEST_1
    ...
    def test_topic_public_with_principal_account_and_organization(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"SNS topic {topic_name} is not public because its policy only allows access from the account {AWS_ACCOUNT_NUMBER} and organization {org_id}."
            )
            assert result[0].resource_id == topic_name
            assert result[0].resource_arn == topic_arn
            assert result[0].region == AWS_REGION_EU_WEST_1
