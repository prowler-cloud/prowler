...
class Test_SNS_Service:
    ...
    def test_list_topics_exception(self, mock_generate_regional_clients):
        ...
        mock_generate_regional_clients.side_effect = lambda provider, service: fake_generate_regional_clients(provider, service)
        aws_provider = set_mocked_aws_provider([AWS_REGION_EU_WEST_1])
        sns = SNS(aws_provider)
    ...
    def test_list_subscriptions_by_topic_unknown_region(self):
        ...
        assert subscription.endpoint == "smtp://endpoint.example.com"from unittest.mock import patch
    ...
    def test_list_topics_with_audit_resources_filter(self, monkeypatch):
        ...
        fake_client.get_paginator = fake_get_paginator.__get__(fake_client, type(fake_client))
        
        # Patch is_resource_filtered to always return False so that the topic is considered not filtered.
        with patch("prowler.providers.aws.services.sns.sns_service.is_resource_filtered", return_value=False):
...
from prowler.providers.aws.services.sns.sns_service import SNS, is_resource_filtered
