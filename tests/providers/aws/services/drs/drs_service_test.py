...
class Test_DRS_Service:
    ...
    def test__list_replication_sets__(self):
        audit_info = self.set_mocked_audit_info()
        ssmincidents = DRS(audit_info)
        assert len(ssmincidents.drs_jobs) == 1
        assert (
            ssmincidents.drs_jobs[0].arn
            == "arn:aws:disaster-recovery:us-east-1:123456789012:job/jobID1"
        )
        assert ssmincidents.drs_jobs[0].id == "jobID1"
        assert ssmincidents.drs_jobs[0].region == AWS_REGION
        assert ssmincidents.drs_jobs[0].tags == [{"test_tag": "test_value"}]
