...
class Test_codebuild_project_uses_allowed_github_organizations:
    ...
    @mock_aws
    def test_project_github_malformed_url(self):
        aws_provider = set_mocked_aws_provider([AWS_REGION_EU_WEST_1])
        codebuild_client = client("codebuild", region_name=AWS_REGION_EU_WEST_1)
        iam_client = client("iam", region_name=AWS_REGION_EU_WEST_1)
        project_name = "test-project-github-malformed-url"
        role_name = "codebuild-test-role"
        role_arn = iam_client.create_role(
            RoleName=role_name,
            AssumeRolePolicyDocument="""{
        ...
        project_arn = codebuild_client.create_project(
            ...
            source={
                "type": "GITHUB",
                "location": "https://github.com/",  # Malformed URL, no org
            },
            artifacts={"type": "NO_ARTIFACTS"},
            environment={
                "type": "LINUX_CONTAINER",
        ...
        with (
            ...
            assert result[0].resource_arn == project_arn
            assert re.search(
                r"invalid GitHub repo URL", result[0].status_extended, re.IGNORECASE
            )
            assert result[0].region == AWS_REGION_EU_WEST_1
