...
class Test_wafv2_cloudwatch_metrics_enabled:
    ...
    def test_wafv2_wb_acl_with_metrics(self):
        wafv2_client = mock.MagicMock
        wafv2_client.web_acls = []
        ...
        with mock.patch(
            ...
            assert (
                ...
                == f"AWS WAFv2 Web ACL {waf_id} has CloudWatch Metrics enabled."
            )
    ...
    def test_wafv2_wb_acl_without_metrics(self):
        wafv2_client = mock.MagicMock
        wafv2_client.web_acls = []
        wafv2_client.enabled = True
        wafv2_client.web_acls.append(
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == f"AWS WAFv2 Web ACL {waf_id} does not have CloudWatch Metrics enabled."
            )
            assert result[0].resource_id == waf_id
            assert result[0].resource_arn == waf_arn
            assert result[0].region == AWS_REGION_EU_WEST_1
