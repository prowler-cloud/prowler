...
class Test_opensearch_service_domains_not_publicly_accessible:
    ...
    def test_domain_inside_vpc_with_public_subnet(self):
        opensearch_client = mock.MagicMock
        opensearch_client.opensearch_domains = []
        vpc_client = mock.MagicMock
        vpc_client.vpc_subnets = {
            "subnet-123456": mock.MagicMock(public=True),
            "subnet-654321": mock.MagicMock(public=False),
        }
        ...
        with mock.patch(
            ...
            "prowler.providers.aws.services.opensearch.opensearch_service_domains_not_publicly_accessible.opensearch_service_domains_not_publicly_accessible.vpc_client",
            new=vpc_client,
        ):
            from prowler.providers.aws.services.opensearch.opensearch_service import (
                OpenSearchDomain,
            ...
            assert (
                result[0].status_extended
                == f"Opensearch domain {domain_name} is publicly accessible due to public subnets: subnet-123456."
            )
            assert result[0].resource_id == domain_name
            assert (
                result[0].resource_arn
