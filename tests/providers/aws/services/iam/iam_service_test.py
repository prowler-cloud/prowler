...
nTTxU4a7x1naFxzYXK1iQ1vMARKMjDb19QEJIEJKZlDK4uS7yMlf1nFS
    ...
    @mock_aws
    def test_has_credentials_no_credentials(self):
        """Test has_credentials when user has no credentials"""
        aws_provider = set_mocked_aws_provider([AWS_REGION_US_EAST_1])
        iam = IAM(aws_provider)
        ...
        has_creds, cred_types = iam.has_credentials(user_data)

        assert has_creds is False
        assert cred_types == []

    @mock_aws
    def test_has_credentials_password_only(self):
        """Test has_credentials when user has only password"""
        aws_provider = set_mocked_aws_provider([AWS_REGION_US_EAST_1])
        iam = IAM(aws_provider)
        ...
        assert cred_types == ["password"]

    @mock_aws
    def test_has_credentials_access_key_1_only(self):
        """Test has_credentials when user has only access key 1"""
        aws_provider = set_mocked_aws_provider([AWS_REGION_US_EAST_1])
        iam = IAM(aws_provider)
        ...
        assert has_creds is True
        assert cred_types == ["access_key_1"]

    @mock_aws
    def test_has_credentials_access_key_2_only(self):
        """Test has_credentials when user has only access key 2"""
        aws_provider = set_mocked_aws_provider([AWS_REGION_US_EAST_1])
        iam = IAM(aws_provider)
        ...
        assert has_creds is True
        assert cred_types == ["access_key_2"]

    @mock_aws
    def test_has_credentials_all_credentials(self):
        """Test has_credentials when user has all credential types"""
        aws_provider = set_mocked_aws_provider([AWS_REGION_US_EAST_1])
        iam = IAM(aws_provider)
        ...
        assert has_creds is True
        assert cred_types == ["password", "access_key_1", "access_key_2"]

    @mock_aws
    def test_has_credentials_password_and_access_key_1(self):
        """Test has_credentials when user has password and access key 1"""
        aws_provider = set_mocked_aws_provider([AWS_REGION_US_EAST_1])
        iam = IAM(aws_provider)
        ...
        assert has_creds is True
        assert cred_types == ["password", "access_key_1"]

    @mock_aws
    def test_has_credentials_both_access_keys(self):
        """Test has_credentials when user has both access keys but no password"""
        aws_provider = set_mocked_aws_provider([AWS_REGION_US_EAST_1])
        iam = IAM(aws_provider)
        ...
        assert has_creds is True
        assert cred_types == ["access_key_1", "access_key_2"]
