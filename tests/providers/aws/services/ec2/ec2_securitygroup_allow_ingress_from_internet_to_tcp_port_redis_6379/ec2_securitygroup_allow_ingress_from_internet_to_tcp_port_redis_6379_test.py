...
from tests.providers.aws.audit_info_utils import (
    AWS_REGION_US_EAST_1_EU_WEST_1,
...
class Test_ec2_securitygroup_allow_ingress_from_internet_to_tcp_port_redis_6379:
    ...
    def test_ec2_default_sgs(self):
        ...
        ec2_client_us_east_1 = client("ec2", region_name=AWS_REGION_US_EAST_1_US_EAST_1)
        ec2_client_us_east_1.create_vpc(CidrBlock="10.0.0.0/16")
        sgs_us_east_1 = ec2_client_us_east_1.describe_security_groups()[
            "SecurityGroups"
        ]

        ec2_client_eu_west_1 = client("ec2", region_name=AWS_REGION_US_EAST_1_EU_WEST_1)
        ...
        with mock.patch(
            ...
            for sg in sgs_us_east_1:
                for res in result:
                    if res.resource_id == sg["GroupId"]:
                        ...
                        assert res.region == AWS_REGION_US_EAST_1_US_EAST_1
                        assert (
                            res.status_extended
                        ...
                        assert (
                            res.resource_arn
                            == f"arn:{current_audit_info.audited_partition}:ec2:{AWS_REGION_US_EAST_1_US_EAST_1}:{current_audit_info.audited_account}:security-group/{sg['GroupId']}"
                        )
            ...
            for sg in sgs_eu_west_1:
                for res in result:
                    if res.resource_id == sg["GroupId"]:
                        assert res.status == "PASS"
                        assert res.region == AWS_REGION_US_EAST_1_EU_WEST_1
                        assert (
                            res.status_extended
                            == f"Security group {sg['GroupName']} ({sg['GroupId']}) does not have Redis port 6379 open to the Internet."
                        )
                        assert (
                            res.resource_arn
                            == f"arn:{current_audit_info.audited_partition}:ec2:{AWS_REGION_US_EAST_1_EU_WEST_1}:{current_audit_info.audited_account}:security-group/{sg['GroupId']}"
                        )
                        assert res.resource_details == sg["GroupName"]
                        assert res.resource_tags == []
        ...
        with mock.patch(
            ...
            for sg in result:
                if sg.resource_id == default_sg_id:
                    ...
                    assert sg.region == AWS_REGION_US_EAST_1_US_EAST_1
                    assert (
                        sg.status_extended
                    ...
                    assert (
                        sg.resource_arn
                        == f"arn:{current_audit_info.audited_partition}:ec2:{AWS_REGION_US_EAST_1_US_EAST_1}:{current_audit_info.audited_account}:security-group/{default_sg_id}"
                    )
    ...
    def test_ec2_default_sgs_ignoring_vpc_in_use(self):
        # Create EC2 Mocked Resources
        ec2 = resource("ec2", region_name=AWS_REGION_US_EAST_1_US_EAST_1)
