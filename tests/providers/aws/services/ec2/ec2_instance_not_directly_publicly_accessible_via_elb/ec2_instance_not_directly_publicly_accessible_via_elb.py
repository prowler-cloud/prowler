...
class Test_ec2_instance_not_directly_publicly_accessible_via_elb:
    ...
    def test_ec2_behind_public_elb(self):
        ...
        security_group_instance = ec2.create_security_group(
            GroupName="sg01_instance", Description="Test security group for EC2 instance"
        )
            ...
            NetworkInterfaces=[
                {
                    ...
                    "Groups": [security_group_instance.id],  # Assign instance to its own security group
                }
            ],
        )[0]
        ...
        with mock.patch(
            ...
            assert findings[0].status_extended == f"EC2 Instance {instance.id} is behind a internet facing classic load balancer my-lb.us-east-1.elb.amazonaws.com."
    ...
    def test_ec2_behind_internal_elb(self):
        ...
        with mock.patch(
            ...
            assert findings[0].status_extended == f"EC2 Instance {instance.id} is not behind a internet facing classic load balancer."
    ...
    def test_two_ec2_behind_public_elb(self):
        ...
        with mock.patch(
            ...
            assert findings[0].status == "FAIL"
            assert findings[0].status_extended == f"EC2 Instance {instances[0].id} is behind a internet facing classic load balancer my-lb.us-east-1.elb.amazonaws.com."
            assert findings[1].status == "FAIL"
            assert findings[1].status_extended == f"EC2 Instance {instances[1].id} is behind a internet facing classic load balancer my-lb.us-east-1.elb.amazonaws.com."