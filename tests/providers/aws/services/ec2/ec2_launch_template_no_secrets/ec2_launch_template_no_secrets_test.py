...
class Test_ec2_launch_template_no_secrets:
    ...
    def test_one_launch_template_with_secrets_in_multiple_versions(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Potential secret found in User Data for EC2 Launch Template {launch_template_name} in template versions: Version 1: Hex High Entropy String on line 3, Version 2: Hex High Entropy String on line 3."
            )
            assert result[0].resource_id == launch_template_id
            assert result[0].region == AWS_REGION_US_EAST_1
            assert result[0].resource_arn == (
    ...
    def test_one_launch_template_with_secrets_in_single_version(self):
        ...
        with (
            ...
            assert (
                result[0].status_extended
                == f"Potential secret found in User Data for EC2 Launch Template {launch_template_name} in template versions: Version 1: Hex High Entropy String on line 3."
            )
            assert result[0].resource_id == launch_template_id
            assert result[0].region == AWS_REGION_US_EAST_1
            assert result[0].resource_arn == (
