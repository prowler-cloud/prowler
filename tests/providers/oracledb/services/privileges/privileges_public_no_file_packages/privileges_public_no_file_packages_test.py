...
CHECK_CLIENT = (
    ...
    "privileges_public_no_file_packages."
    "privileges_public_no_file_packages.privileges_client"
)
...
def _run_check(client):
    with (
        ...
        from prowler.providers.oracledb.services.privileges.privileges_public_no_file_packages.privileges_public_no_file_packages import (
            privileges_public_no_file_packages,
        )

        return privileges_public_no_file_packages().execute()
...
class Test_privileges_public_no_file_packages:
    ...
    def test_fail_when_package_is_public(self):
        findings = _run_check(
            _build_client(execute_packages=["DBMS_OUTPUT", "UTL_FILE"])
        )
        assert len(findings) == 1
        assert findings[0].status == "FAIL"
        assert "UTL_FILE" in findings[0].status_extended
