...
CHECK_CLIENT = (
    ...
    "users_password_verify_function_configured."
    "users_password_verify_function_configured.users_client"
)
...
def _run_check(client):
    with (
        ...
        from prowler.providers.oracledb.services.users.users_password_verify_function_configured.users_password_verify_function_configured import (
            users_password_verify_function_configured,
        )

        return users_password_verify_function_configured().execute()
...
class Test_users_password_verify_function_configured:
    ...
    def test_pass(self):
        ...
        assert (
            "uses the password verification function ORA12C_VERIFY_FUNCTION"
            in findings[0].status_extended
        )

    def test_fail(self):
        findings = _run_check(_build_client([_user(password_verify_function="NULL")]))
        assert len(findings) == 1
        assert findings[0].status == "FAIL"
        assert findings[0].resource_name == "APPUSER"
        assert "no password verification function" in findings[0].status_extended
