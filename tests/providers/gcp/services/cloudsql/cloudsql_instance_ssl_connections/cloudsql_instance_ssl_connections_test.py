...
class Test_cloudsql_instance_ssl_connections:
    ...
    def test_cloudsql_instance_ssl_connections_enabled_and_ssl_mode_encrypted(self):
        ...
        with mock.patch(
            ...
            assert (
                ...
                == "Database Instance instance1 requires SSL connections and allows only encrypted connections."
            )
    ...
    def test_cloudsql_instance_ssl_connections_disabled_and_ssl_mode_encrypted(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == "Database Instance instance1 does not require SSL connections but allows only encrypted connections."
            )
            assert result[0].resource_id == "instance1"
            assert result[0].resource_name == "instance1"
            assert result[0].location == GCP_EU1_LOCATION
    ...
    def test_cloudsql_instance_ssl_connections_enabled_and_ssl_mode_not_encrypted(self):
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == "Database Instance instance1 requires SSL connections but allows unencrypted connections."
            )
            assert result[0].resource_id == "instance1"
            assert result[0].resource_name == "instance1"
            assert result[0].location == GCP_EU1_LOCATION
    ...
    def test_cloudsql_instance_ssl_connections_disabled_and_ssl_mode_not_encrypted(
        ...
        with mock.patch(
            ...
            assert (
                result[0].status_extended
                == "Database Instance instance1 does not require SSL connections and allows unencrypted connections."
            )
            assert result[0].resource_id == "instance1"
            assert result[0].resource_name == "instance1"
            assert result[0].location == GCP_EU1_LOCATION
