...
class Test_lts_log_group_retention:
    ...
    def test_log_group_unset_retention(self):
        lts_client = mock.MagicMock()
        lts_client.log_groups = [
            mock.MagicMock(
                log_group_id="group-003",
                log_group_name="unset-retention-logs",
                ttl_in_days=None,
                region="la-south-2",
            )
        ]
        ...
        with (
            ...
            assert len(result) == 1
            assert result[0].status == "FAIL"
            assert "unset-retention-logs" in result[0].status_extended
