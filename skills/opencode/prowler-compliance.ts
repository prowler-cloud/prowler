...
description: Creates and manages Prowler compliance frameworks. Maps security checks to compliance requirements (CIS, NIST, PCI-DSS, SOC2, GDPR, etc.).
license: Apache 2.0
---
...
export default tool({
  description: SKILL,
  args: {
    provider: tool.schema.string().describe("Cloud provider: aws, azure, gcp, kubernetes, github, m365"),
    framework: tool.schema.string().describe("Framework name: cis, nist_800_53, pci_dss, soc2, gdpr, hipaa, fedramp"),
  },
  async execute(args) {
    return `
Prowler Compliance Framework for: ${args.provider} - ${args.framework}

File location: prowler/compliance/${args.provider}/${args.framework}_${args.provider}.json

Steps to create/update:
1. Check existing framework: prowler/compliance/${args.provider}/
2. Create JSON following the Compliance model structure
3. Map requirements to existing checks
4. Test: poetry run python prowler-cli.py ${args.provider} --compliance ${args.framework}

Existing checks for ${args.provider}:
  poetry run python prowler-cli.py ${args.provider} --list-checks

Existing frameworks for ${args.provider}:
  poetry run python prowler-cli.py ${args.provider} --list-compliance

Remember:
- Use original framework requirement IDs
- Map to existing checks when possible
- Document manual checks (no automated validation)
- Include framework version in metadata
    `.trim()
  },
})
