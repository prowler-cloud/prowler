    ...
    try:
        ...
        if check_findings:
            for finding in check_findings:
                ...
                if file_descriptors:
                    add_finding_to_outputs(
                        finding, output_options, audit_info, file_descriptors
                    )
...
def add_finding_to_outputs(finding, output_options, audit_info, file_descriptors):
    # AWS specific outputs
    if finding.check_metadata.Provider == "aws":
        if "ens_rd2022_aws" in output_options.output_modes or "cis" in str(
            output_options.output_modes
        ):
            fill_compliance(output_options, finding, audit_info, file_descriptors)
        ...
        if output_options.security_hub_enabled and finding.status != "INFO":
