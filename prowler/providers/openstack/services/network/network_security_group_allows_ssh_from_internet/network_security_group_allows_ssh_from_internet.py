    ...
    def execute(self) -> List[CheckReportOpenStack]:
        ...
        for sg in network_client.security_groups:
            ...
            if ssh_exposed:
                ...
                report.status_extended = f"Security group {sg.name} ({sg.id}) allows unrestricted SSH access (port 22) from the Internet via {rules_str}."  # noqa: E501
            else:
                report.status = "PASS"
                report.status_extended = f"Security group {sg.name} ({sg.id}) does not allow SSH (port 22) from the Internet."  # noqa: E501
