                    ...
                    if 'Contents' in result:
                        for obj in result['Contents']:
                            ...
                            try:
                                ...
                                logger.error(f"Failed to decode content for {obj['Key']}: {e}")
                                continue
                            ...
                            if content:
                                try:
                                    ...
                                    logger.error(f"Failed to decode JSON for {obj['Key']}: {e}")
                                    continue

                                for event in events:
                                    if any(pattern in json.dumps(event) for pattern in secret_patterns):
                                        ...
                                        report.status_extended = (
                                            f"Trail {trail.name} contains events with secret patterns in log file {obj['Key']}."
                                        )
