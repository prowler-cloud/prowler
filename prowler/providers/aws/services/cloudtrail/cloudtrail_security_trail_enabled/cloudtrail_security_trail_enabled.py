...
class cloudtrail_security_trail_enabled(Check):
    def execute(self):
        ...
        if cloudtrail_client.trails:
            for trail in cloudtrail_client.trails.values():
                if trail.name:
                    ...
                    if not trail.is_multiregion:  # is_multiregion typically implies global service event recording
                        report.status = "FAIL"
                    ...
                    if not trail.is_multiregion:
                        report.status = "FAIL"
                        report.status_extended = f"CloudTrail {trail.name} is not a multi-region trail."
                    ...
                    if not trail.kms_key:
                        report.status = "FAIL"
                        report.status_extended = f"CloudTrail {trail.name} is not encrypted with a KMS key."

                    # Check if the trail records events for reads and writes
                    records_read_write_events = any(
                        selector.event_selector.get('ReadWriteType') in ['All', 'ReadOnly', 'WriteOnly']
                    ...
                    if excludes_management_events:
                        report.status = "FAIL"
                        report.status_extended = f"CloudTrail {trail.name} excludes some management events."
        ...
        if not any(finding.status == "PASS" for finding in findings):
            ...
            overall_report.status_extended = "No CloudTrail trail meets all security best practices."
