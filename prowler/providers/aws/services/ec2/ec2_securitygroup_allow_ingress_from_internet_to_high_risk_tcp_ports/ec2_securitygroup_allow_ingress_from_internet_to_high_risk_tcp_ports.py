                ...
                if not ec2_client.is_failed_check(
                    ...
                    for ingress_rule in security_group.ingress_rules:
                        if check_security_group(
                            ...
                            report.status = "FAIL"
                            report.status_extended = f"Security group {security_group.name} ({security_group.id}) has a high risk port open to the Internet."
                            break
                else:
                    report.status_extended = f"Security group {security_group.name} ({security_group.id}) has all ports open to the Internet and therefore was not checked against any of the high risk ports (25,110,135,143,445,3000,4333,5000,5500,8080,8088)."
