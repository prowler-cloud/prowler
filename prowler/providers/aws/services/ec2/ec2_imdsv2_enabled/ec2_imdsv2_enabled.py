    ...
    def execute(self):
        ...
        for instance_metadata_default in ec2_client.instance_metadata_defaults:
            if (
                ...
                report.resource_arn = f"arn:aws:ec2:{instance_metadata_default.region}:{ec2_client.audited_account}"
                report.resource_id = ec2_client.audited_account
                if instance_metadata_default.http_tokens == "required":
                    report.status = "PASS"
                    report.status_extended = f"IMDSv2 is enabled for region {instance_metadata_default.region}."
                else:
                    report.status = "FAIL"
                    report.status_extended = f"IMDSv2 is not enabled for region {instance_metadata_default.region}."
