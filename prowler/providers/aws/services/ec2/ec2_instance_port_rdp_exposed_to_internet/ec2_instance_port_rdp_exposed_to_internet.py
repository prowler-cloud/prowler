                            ...
                            for ingress_rule in sg.ingress_rules:
                                if check_security_group(
                                    ...
                                    if instance.public_ip:
                                        report.status_extended = f"Instance {instance.id} has RDP exposed to 0.0.0.0/0 on public ip address {instance.public_ip} but in private subnet {instance.subnet_id}."
                                        report.check_metadata.Severity = "high"
                                        if vpc_client.vpc_subnets[
                                            instance.subnet_id
                                        ].public:
                                            report.status_extended = f"Instance {instance.id} has RDP exposed to 0.0.0.0/0 on public ip address {instance.public_ip} in public subnet {instance.subnet_id}."
                                            report.check_metadata.Severity = "critical"
                                    else:
                                        report.status_extended = f"Instance {instance.id} has RDP exposed to 0.0.0.0/0 but with no public ip address."
