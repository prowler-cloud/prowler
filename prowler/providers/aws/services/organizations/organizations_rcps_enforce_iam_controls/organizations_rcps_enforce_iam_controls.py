...
class organizations_rcps_enforce_iam_controls(Check):
    ...
    def _policy_enforces_iam_controls(self, policy):
        ...
        iam_actions = [
            ...
            "iam:detach", "iam:add", "iam:remove", "iam:pass", "iam:getuser",
            "iam:addrole", "iam:createuser", "iam:createlogin", "iam:createaccesskey"
        ]
        ...
        for statement in statements:
            ...
            if statement.get("Effect") == "Deny":
                ...
                for action in actions:
                    ...
                    if action.startswith("iam:") or any(iam_action in action for iam_action in iam_actions):
                        return True
            ...
            if "Resource" in statement and "Condition" in statement:
                ...
                if isinstance(resource, str) and ("iam" in resource.lower() or "role" in resource.lower() or "user" in resource.lower()):
                    # Check conditions for permissions boundaries
                    condition_str = str(condition).lower()
                    if "permissionsboundary" in condition_str or "aws:principalarn" in condition_str:
                        return True
