    ...
    def execute(self):
        ...
        for cluster in redshift_client.clusters:
            ...
            if cluster.logging_enabled:
                logging_status = "PASS"
                logging_status_extended = (
                    f"Audit logging is enabled for Redshift cluster {cluster.id}."
                )
            else:
                logging_status = "FAIL"
                logging_status_extended = (
                    f"Audit logging is NOT enabled for Redshift cluster {cluster.id}."
                )
            ...
            if encryption_status == "PASS" and logging_status == "PASS":
                ...
                report.status_extended = (
                    f"{encryption_status_extended} {logging_status_extended}"
                )
            else:
                report.status = "FAIL"
                report.status_extended = (
