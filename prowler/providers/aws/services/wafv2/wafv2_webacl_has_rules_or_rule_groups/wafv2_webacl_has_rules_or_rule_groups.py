...
class wafv2_webacl_has_rules_or_rule_groups(Check):
    def execute(self):
        findings = []
        for web_acl in wafv2_client.web_acls.values():
            ...
            report.status = "FAIL"
            report.status_extended = f"AWS WAFv2 Web ACL {web_acl.id} does not have any rules or rule groups attached."

            if web_acl.rules or web_acl.rule_groups:
                report.status = "PASS"
                report.status_extended = f"AWS WAFv2 Web ACL {web_acl.id} does have rules or rule groups attached."
