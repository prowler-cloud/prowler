...
class bedrock_guardrails_configured_and_used(Check):
    """Ensure Bedrock guardrails are configured and actively used.

    This check verifies that at least one Amazon Bedrock guardrail is configured
    in each audited region to prevent harmful content in model invocations.
    ...
    def execute(self) -> list[Check_Report_AWS]:
        ...
        """
        findings = []
        for region in bedrock_client.regional_clients:
            report = Check_Report_AWS(
                metadata=self.metadata(),
                resource=bedrock_client.logging_configurations.get(region),
            )
            report.region = region
            report.resource_id = "bedrock-guardrails"
            report.resource_arn = f"arn:{bedrock_client.audited_partition}:bedrock:{region}:{bedrock_client.audited_account}:guardrail"
            ...
            if regional_guardrails:
                ...
                report.status = "FAIL"
                report.status_extended = f"Bedrock has no guardrails configured in region {region}."
