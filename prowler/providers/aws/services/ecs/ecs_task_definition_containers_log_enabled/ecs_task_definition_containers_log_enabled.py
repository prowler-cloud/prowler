...
class ecs_task_definition_containers_log_enabled(Check):
    def execute(self):
        findings = []
        for task_definition in ecs_client.task_definitions.values():
            ...
            report.status = "PASS"
            report.status_extended = f"ECS task definition {task_definition.name} containers have logging configured."
            failed_containers = []
            ...
            if failed_containers:
                report.status_extended = f"ECS task definition {task_definition.name} has containers running with no logging configuration: {', '.join(failed_containers)}"
