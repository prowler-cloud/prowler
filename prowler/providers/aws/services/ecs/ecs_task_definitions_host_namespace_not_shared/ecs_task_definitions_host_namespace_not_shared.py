...
class ecs_task_definitions_host_namespace_not_shared(Check):
    def execute(self):
        ...
        for task_definition in ecs_client.task_definitions.values():
            ...
            report.status = "PASS"
            report.status_extended = f"ECS task definition '{task_definition.name}' does not share a host's process namespace with its containers."
            if task_definition.pid_mode == "host":
                report.status = "FAIL"
                report.status_extended = f"ECS task definition '{task_definition.name}' is configured to share a host's process namespace with its containers."
