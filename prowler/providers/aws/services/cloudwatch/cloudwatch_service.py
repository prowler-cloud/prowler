...
class Logs:
    def __init__(self, audit_info):
        ...
        if "cloudwatch_no_secerts_in_logs" in audit_info.audit_metadata.expected_checks:
            # Can look at defining these parameters in the config file
            self.number_of_log_streams_to_retrieve = (
                2  # The number of log streams to retrieve for each log group.
            )
        ...
        try:
            ...
            for count, log_group in enumerate(regional_log_groups, start=1):
                ...
                for log_stream in log_streams["logStreams"]:
                    ...
                    logger.info(
                        f"CloudWatch Logs - Found {len(events)} log events for {log_stream_name} stream for log group {log_group.name} in {regional_client.region}..."
                    )
