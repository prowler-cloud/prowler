    ...
    def execute(self):
        ...
        for api in appsync_client.graphqlapis.values():
            if api.type == "GRAPHQL":
                ...
                report.status = "PASS"
                report.status_extended = f"AppSync GraphqlApi {api.name} is not using an API KEY for authentication."
                if api.authentication_type == "API_KEY":
                    report.status = "FAIL"
                    report.status_extended = f"AppSync GraphqlApi {api.name} is using an API KEY for authentication."
