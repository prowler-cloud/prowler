        ...
        for state_machine in stepfunctions_client.state_machines.values():
            ...
            if state_machine.logging_configuration.level == LoggingLevel.OFF:
                ...
                if valid_config_value:
                    report.status = "FAIL"
                    report.status_extended = f"Step Functions state machine '{state_machine.name}' have the log level '{state_machine.logging_configuration.level}' which does not match the one specified in the configuration '{stepfunctions_client.audit_config.get('statemachines_log_level')}'."
                else:
