...
class s3_bucket_public_write_acl(Check):
    """Ensure S3 bucket ACLs do not let anyone write to the bucket.

    - PASS: Public access is blocked, or the ACL grants no public write access.
    - FAIL: The ACL grants AllUsers or AuthenticatedUsers write access.
    - MANUAL: The bucket ACL could not be retrieved (missing permissions) and
      neither the account nor the bucket ignores public ACLs.
    """
            ...
            for bucket in s3_client.buckets.values():
                if bucket.public_access_block:
                    ...
                    if not (
                        ...
                        acls_ignored = bucket.public_access_block.ignore_public_acls or (
                            s3control_client.account_public_access_block
                            and s3control_client.account_public_access_block.ignore_public_acls
                        )
                        if not bucket.acl_retrieved and not acls_ignored:
                            report.status = "MANUAL"
                            report.status_extended = f"Cannot evaluate whether S3 Bucket {bucket.name} is publicly writable: the bucket ACL could not be retrieved. Verify that the scanning credentials are allowed to call s3:GetBucketAcl."
