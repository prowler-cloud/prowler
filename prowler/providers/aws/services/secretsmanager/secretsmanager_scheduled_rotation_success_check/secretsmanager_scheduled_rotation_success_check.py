            ...
            if secret.rotation_enabled:
                try:
                    ...
                    if (
                        ...
                        and next_rotation_date > last_rotated_date
                    ):
                        report.status = "PASS"
                        report.status_extended = f"SecretsManager secret {secret.name} has successfully rotated according to the schedule."
                    else:
                        report.status = "FAIL"
                        report.status_extended = f"SecretsManager secret {secret.name} did not rotate as scheduled."
                except Exception as e:
                    report.status = "FAIL"
                    report.status_extended = f"Failed to retrieve rotation details for secret {secret.name}: {str(e)}"
            else:
                report.status = "MANUAL"
                report.status_extended = f"SecretsManager secret {secret.name} does not have rotation enabled. Manual review required."
