        ...
        for region in sorted(sagemaker_client.processing_jobs_scanned_regions):
            clarify_jobs = sorted(
                (
                    ...
                    and job.image_uri
                    and "clarify" in job.image_uri.lower()
                ),
            ...
            if clarify_jobs:
                for job in clarify_jobs:
                    report = Check_Report_AWS(
                        metadata=self.metadata(), resource=job
                    )
                    report.status = "PASS"
                ...
                report.status = "FAIL"
                report.status_extended = f"No SageMaker Clarify processing jobs found in region {region}."
                findings.append(report)
