...
class iam_root_mfa_enabled(Check):
    def execute(self) -> Check_Report_AWS:
        ...
        # Check if root credentials are managed by AWS Organizations
        is_org_managed = (
            iam_client.organization_features is not None
            and "RootCredentialsManagement" in iam_client.organization_features
        )
                    ...
                    if user["mfa_active"] == "false":
                        ...
                        if is_org_managed:
                            report.status_extended = (
                                ...
                                "Root credentials are managed by AWS Organizations - "
                                "verify MFA settings at the organization level."
                            )
                        else:
                            report.status_extended = (
                                "MFA is not enabled for root account."
