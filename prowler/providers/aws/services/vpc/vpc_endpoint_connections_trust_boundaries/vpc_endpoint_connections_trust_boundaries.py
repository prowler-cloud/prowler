...
class vpc_endpoint_connections_trust_boundaries(Check):
    def execute(self):
        ...
        for endpoint in vpc_client.vpc_endpoints:
            # Skip endpoint policies that AWS explicitly reports as unsupported,
            # along with the existing unmodifiable PrivateLink service endpoints.
            if (
                endpoint.policy_document
                and "com.amazonaws.vpce." not in endpoint.service_name
                and endpoint.vpc_endpoint_policy_supported is not False
            ):
