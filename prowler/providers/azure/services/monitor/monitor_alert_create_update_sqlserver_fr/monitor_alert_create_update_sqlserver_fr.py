...
class monitor_alert_create_update_sqlserver_fr(Check):
    def execute(self) -> Check_Report_Azure:
        ...
        for (
            ...
            report.status_extended = f"There is not an alert for Create/Update SQL Server Firewall Rule in subscription {subscription_name}."
            for alert_rule in activity_log_alerts:
                if (
                    ...
                    report.status = "PASS"
                    report.status_extended = f"Alert {alert_rule.name} is configured to trigger when a fireall rule for SQL Server is created or updated in subscription {subscription_name}."
