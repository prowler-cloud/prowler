...
class monitor_alert_delete_policy_assignment(Check):
    def execute(self) -> Check_Report_Azure:
        ...
        for (
            ...
            report.status_extended = f"There is not an alert for Delete Policy Assingment in subscription {subscription_name}."
            for alert_rule in activity_log_alerts:
                if (
                    ...
                    report.status = "PASS"
                    report.status_extended = f"Alert {alert_rule.name} is configured to trigger when a policy assignment is deleted in subscription {subscription_name}."
