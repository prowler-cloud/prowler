        ...
        for (
            ...
            for alert_rule in activity_log_alerts:
                if check_alert_rule(
                    ...
                    report.status = "PASS"
                    report.status_extended = f"There is an alert configured for deleting Security Solution in subscription {subscription_name} ({subscription_id})."
                    break
            else:
                ...
                report.status = "FAIL"
                report.status_extended = f"There is not an alert for deleting Security Solution in subscription {subscription_name} ({subscription_id})."
