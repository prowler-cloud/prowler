    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription_id, clusters in aks_client.clusters.items():
            for cluster in clusters.values():
                ...
                report.status = "FAIL"
                report.status_extended = f"Public access to nodes is enabled for cluster '{cluster.name}' in subscription '{subscription_id}'"
                ...
                if cluster.private_fqdn:
                    for agent_pool in cluster.agent_pool_profiles:
                        if not getattr(agent_pool, "enable_node_public_ip", False):
                            report.status = "PASS"
                            report.status_extended = f"Public access to nodes is disabled for cluster '{cluster.name}' in subscription '{subscription_id}'"
