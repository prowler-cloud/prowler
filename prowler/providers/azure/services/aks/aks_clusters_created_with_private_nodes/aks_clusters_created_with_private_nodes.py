    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription_id, clusters in aks_client.clusters.items():
            for cluster in clusters.values():
                ...
                report.status = "PASS"
                report.status_extended = f"Cluster '{cluster.name}' was created with private nodes in subscription '{subscription_id}'"

                for agent_pool in cluster.agent_pool_profiles:
                    if getattr(agent_pool, "enable_node_public_ip", True):
                        report.status = "FAIL"
                        report.status_extended = f"Cluster '{cluster.name}' was not created with private nodes in subscription '{subscription_id}'"
                        break
