    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription_id, clusters in aks_client.clusters.items():
            for cluster_id, cluster in clusters.items():
                ...
                report.status = "PASS"
                report.status_extended = f"Network policy is enabled for cluster '{cluster.name}' in subscription '{subscription_id}'."

                if not getattr(cluster, "network_policy", False):
                    report.status = "FAIL"
                    report.status_extended = f"Network policy is not enabled for cluster '{cluster.name}' in subscription '{subscription_id}'."
