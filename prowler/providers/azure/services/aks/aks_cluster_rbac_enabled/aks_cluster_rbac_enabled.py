    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription_id, clusters in aks_client.clusters.items():
            for cluster in clusters.values():
                ...
                report.status = "PASS"
                report.status_extended = f"RBAC is enabled for cluster '{cluster.name}' in subscription '{subscription_id}'."

                if not cluster.rbac_enabled:
                    report.status = "FAIL"
                    report.status_extended = f"RBAC is not enabled for cluster '{cluster.name}' in subscription '{subscription_id}'."
