    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription, sql_servers in sqlserver_client.sql_servers.items():
            ...
            for sql_server in sql_servers:
                ...
                report.status = "PASS"
                report.status_extended = f"SQL Server {sql_server.name} from subscription {subscription_name} ({subscription}) does not have firewall rules allowing 0.0.0.0-255.255.255.255."
                for firewall_rule in sql_server.firewall_rules:
                    if (
                        ...
                        report.status = "FAIL"
                        report.status_extended = f"SQL Server {sql_server.name} from subscription {subscription_name} ({subscription}) has firewall rules allowing 0.0.0.0-255.255.255.255."
                        break
