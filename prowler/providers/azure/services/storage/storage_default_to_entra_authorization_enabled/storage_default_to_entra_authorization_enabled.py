    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription, storage_accounts in storage_client.storage_accounts.items():
            ...
            for storage_account in storage_accounts:
                ...
                report.status = "FAIL"
                report.status_extended = f"Default to Microsoft Entra authorization is not enabled for storage account {storage_account.name} from subscription {subscription_name} ({subscription})."

                if storage_account.default_to_entra_authorization:
                    report.status = "PASS"
                    report.status_extended = f"Default to Microsoft Entra authorization is enabled for storage account {storage_account.name} from subscription {subscription_name} ({subscription})."
