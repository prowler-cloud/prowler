    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for (
            ...
            for server in servers.values():
                ...
                report.status = "FAIL"
                report.status_extended = f"SSL connection is disabled for server {server.name} in subscription {subscription_name} ({subscription_id})."
                ...
                if "require_secure_transport" in server.configurations:
                    ...
                    if (
                        ...
                        report.status = "PASS"
                        report.status_extended = f"SSL connection is enabled for server {server.name} in subscription {subscription_name} ({subscription_id})."
