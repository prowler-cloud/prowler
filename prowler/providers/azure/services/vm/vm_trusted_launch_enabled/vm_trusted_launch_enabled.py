    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription_id, vms in vm_client.virtual_machines.items():
            for vm in vms.values():
                ...
                report.status = "FAIL"
                report.status_extended = f"VM {vm.resource_name} has trusted launch disabled in subscription {subscription_id}"
                ...
                if (
                    ...
                    report.status = "PASS"
                    report.status_extended = f"VM {vm.resource_name} has trusted launch enabled in subscription {subscription_id}"
