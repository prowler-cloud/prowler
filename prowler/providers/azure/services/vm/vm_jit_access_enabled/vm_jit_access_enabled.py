        ...
        for subscription_id, vms in vm_client.virtual_machines.items():
            ...
            for vm in vms.values():
                ...
                if vm.resource_id.lower() in {
                    ...
                    report.status = "PASS"
                    report.status_extended = f"VM {vm.resource_name} in subscription {subscription_name} ({subscription_id}) has JIT (Just-in-Time) access enabled."
                else:
                    report.status = "FAIL"
                    report.status_extended = f"VM {vm.resource_name} in subscription {subscription_name} ({subscription_id}) does not have JIT (Just-in-Time) access enabled."
