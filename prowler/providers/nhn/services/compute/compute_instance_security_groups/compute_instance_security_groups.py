...
class compute_instance_security_groups(Check):
    def execute(self):
        ...
        for instance in compute_client.instances:
            ...
            report.status = "PASS"
            report.status_extended = f"VM Instance {instance.name} has a variety of security groups."
            if instance.security_groups:
                report.status = "FAIL"
                report.status_extended = f"VM Instance {instance.name} has only the default security group."
