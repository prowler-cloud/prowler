...
class storage_bucket_lifecycle_configured(Check):
    """Check if E2E Cloud object storage buckets have lifecycle configuration enabled."""
    ...
    def execute(self) -> list[CheckReportE2e]:
        ...
        for bucket in storage_client.buckets:
            ...
            report.status_extended = (
                f"Object storage bucket {bucket.name} has lifecycle configuration enabled."
            )
            if bucket.lifecycle_configuration_status != "Configured":
                report.status = "FAIL"
                report.status_extended = (
                    f"Object storage bucket {bucket.name} does not have lifecycle configuration enabled."
                )
