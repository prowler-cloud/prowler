from prowler.lib.check.models import Check, CheckReportLinode
from prowler.providers.linode.services.firewall.firewall_client import firewall_client

INTERNET_CIDRS = {"0.0.0.0/0", "::/0"}


def _rule_allows_ssh_from_internet(rule) -> bool:
    """Check if a rule allows SSH (port 22) from the internet."""
    if rule.action != "ACCEPT":
        return False
    if rule.protocol not in ("TCP", "ALL"):
        return False
    # Check if port 22 is included
    if rule.ports:
        ports_str = rule.ports.strip()
        if ports_str == "22":
            pass
        elif "-" in ports_str:
            try:
                start, end = ports_str.split("-", 1)
                if not (int(start) <= 22 <= int(end)):
                    return False
            except ValueError:
                return False
        elif "," in ports_str:
            if "22" not in [p.strip() for p in ports_str.split(",")]:
                return False
        else:
            # Single port that isn't 22
            try:
                if int(ports_str) != 22:
                    return False
            except ValueError:
                return False
    # Empty ports means all ports

    # Check if source is internet
    all_addresses = rule.addresses_ipv4 + rule.addresses_ipv6
    for addr in all_addresses:
        if addr in INTERNET_CIDRS:
            return True
    return False


class firewall_allows_ssh_from_internet(Check):
    """Check if Linode firewalls allow SSH (port 22) from the internet."""

    def execute(self) -> list[CheckReportLinode]:
        findings = []

        for fw in firewall_client.firewalls:
            report = CheckReportLinode(
                metadata=self.metadata(),
                resource=fw,
                resource_name=fw.label,
                resource_id=str(fw.id),
                region="global",
            )
            report.resource_tags = fw.tags

            ssh_open = any(
                _rule_allows_ssh_from_internet(rule) for rule in fw.inbound_rules
            )

            if ssh_open:
                report.status = "FAIL"
                report.status_extended = (
                    f"Firewall '{fw.label}' allows SSH (port 22) from the internet."
                )
            else:
                report.status = "PASS"
                report.status_extended = (
                    f"Firewall '{fw.label}' does not allow SSH from the internet."
                )

            findings.append(report)

        return findings
