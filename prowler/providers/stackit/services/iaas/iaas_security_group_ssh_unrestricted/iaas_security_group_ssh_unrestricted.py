    ...
    def execute(self):
        ...
        for security_group in iaas_client.security_groups:
            ...
            for rule in security_group.rules:
                ...
                if (
                    ...
                    if rule.includes_port(22):
                        ...
                        report.status_extended = (
                            ...
                            f"from {rule.ip_range} via rule '{rule.id}'."
                        )
                ...
                # No unrestricted SSH rule found - PASS
                report = CheckReportStackIT(
                    metadata=self.metadata(),
                    resource=security_group,
                )
                ...
                report.status = "PASS"
                report.status_extended = (
                    f"Security group '{security_group.name}' does not allow unrestricted SSH access."
                )
                report.resource_id = security_group.id
                report.resource_name = security_group.name
                report.location = security_group.region
