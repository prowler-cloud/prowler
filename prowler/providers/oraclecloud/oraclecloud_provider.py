...
class OraclecloudProvider(Provider):
    ...
    def __init__(
        ...
        if self._regions:
            self._home_region = next((region.key for region in self._regions if region.is_home_region), self._regions[0])
        else:
            self._home_region = "us-ashburn-1"
        logger.warn(f"Home region is: {self._home_region}")
        # Get compartments
