...
Ensures that the RAM password policy requires at least one symbol (special character).
...
class ram_password_require_symbols(Check):
    """Check if the RAM password policy requires at least one symbol"""

    def execute(self):
        """Execute the ram_password_require_symbols check"""
        findings = []
        ...
        if ram_client.password_policy and ram_client.password_policy.require_symbols:
            report.status = "PASS"
            report.status_extended = "RAM password policy requires at least one symbol (special character)."
        else:
            report.status = "FAIL"
            report.status_extended = "RAM password policy does not require symbols. Enable the symbol requirement to increase password complexity."
