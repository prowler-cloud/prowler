        ...
        for user_arn, user in ram_client.users.items():
            for access_key in user.access_keys:
                report = Check_Report_AlibabaCloud(metadata=self.metadata(), resource=type('obj', (object,), {
                    ...
                    'name': f"{user.name}-{access_key.access_key_id}",
                ...
                report.resource_arn = f"{user.arn}/access-key/{access_key.access_key_id}"

                # Check if access key is older than 90 days
                if access_key.create_date:
                    try:
                        create_date = datetime.fromisoformat(access_key.create_date.replace('Z', '+00:00'))
