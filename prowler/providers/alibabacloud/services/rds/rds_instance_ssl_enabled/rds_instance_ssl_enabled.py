...
class rds_instance_ssl_enabled(Check):
    def execute(self):
        ...
        for db_instance in rds_client.db_instances.values():
            ...
            report.status = "FAIL"
            report.status_extended = f"RDS instance {db_instance.db_instance_name} does not have SSL enabled."
            if db_instance.ssl_enabled:
                report.status = "PASS"
                report.status_extended = (
                    f"RDS instance {db_instance.db_instance_name} has SSL enabled."
                )
