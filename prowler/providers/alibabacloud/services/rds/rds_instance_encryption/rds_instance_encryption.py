...
class rds_instance_encryption(Check):
    def execute(self):
        ...
        for db_instance in rds_client.db_instances.values():
            ...
            report.status = "FAIL"
            report.status_extended = f"RDS instance {db_instance.db_instance_name} does not have encryption enabled."
            if db_instance.encryption_enabled:
                report.status = "PASS"
                report.status_extended = f"RDS instance {db_instance.db_instance_name} has encryption enabled."
