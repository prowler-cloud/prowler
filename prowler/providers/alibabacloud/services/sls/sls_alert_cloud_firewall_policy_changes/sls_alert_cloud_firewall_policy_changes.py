...
class sls_alert_cloud_firewall_policy_changes(Check):
    def execute(self):
        ...
        report.status_extended = (
            "No enabled alerts found for monitoring cloud firewall policy changes."
        )
        if len(firewall_policy_alerts) > 0:
            alert_names = [alert.display_name for alert in firewall_policy_alerts]
            report.status = "PASS"
            report.status_extended = f"Found {len(firewall_policy_alerts)} enabled alert(s) monitoring cloud firewall policy changes: {', '.join(alert_names)}."
