"""
Check: sls_alert_ram_policy_changes

Ensures that an alert is configured in SLS to monitor and notify on RAM policy changes.
...
class sls_alert_ram_policy_changes(Check):
    """Check if an alert is configured for ram policy changes"""

    def execute(self):
        """Execute the sls_alert_ram_policy_changes check"""
        findings = []

        # Check if there's at least one alert monitoring ram policy changes
        matching_alerts = []
        for alert_arn, alert in sls_client.alerts.items():
            if "ram" in alert.display_name.lower() and "policy" in alert.display_name.lower():
        ...
        if len(matching_alerts) > 0:
            ...
            report.status = "PASS"
            report.status_extended = f"Found {len(matching_alerts)} enabled alert(s) monitoring ram policy changes: {', '.join(alert_names)}."
        else:
            report.status = "FAIL"
            report.status_extended = "No enabled alerts found for monitoring ram policy changes. Create and enable an alert to detect configuration changes."
