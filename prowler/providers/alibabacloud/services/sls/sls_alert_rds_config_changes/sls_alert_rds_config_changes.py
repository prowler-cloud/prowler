...
Ensures that an alert is configured in SLS to monitor and notify on RDS instance configuration changes.
...
class sls_alert_rds_config_changes(Check):
    """Check if an alert is configured for rds configuration changes"""

    def execute(self):
        """Execute the sls_alert_rds_config_changes check"""
        findings = []

        # Check if there's at least one alert monitoring rds configuration changes
        matching_alerts = []
        for alert_arn, alert in sls_client.alerts.items():
            if "rds" in alert.display_name.lower() and "config" in alert.display_name.lower():
        ...
        if len(matching_alerts) > 0:
            ...
            report.status = "PASS"
            report.status_extended = f"Found {len(matching_alerts)} enabled alert(s) monitoring rds configuration changes: {', '.join(alert_names)}."
        else:
            report.status = "FAIL"
            report.status_extended = "No enabled alerts found for monitoring rds configuration changes. Create and enable an alert to detect configuration changes."
