    ...
    def execute(self) -> list[Check_Report_Kubernetes]:
        ...
        for pod in core_client.pods.values():
            ...
            report.status_extended = (
                f"Pod {pod.name} regular containers use fixed image tags."
            )

            for container in (pod.containers or {}).values():
                if not _has_fixed_image(container.image):
                    report.status = "FAIL"
                    report.status_extended = f"Pod {pod.name} container {container.name} uses an unfixed image tag: {container.image}."
                    break
