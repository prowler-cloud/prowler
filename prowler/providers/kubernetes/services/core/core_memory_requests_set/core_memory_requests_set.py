    ...
    def execute(self) -> list[Check_Report_Kubernetes]:
        ...
        for pod in core_client.pods.values():
            ...
            report.status_extended = (
                f"Pod {pod.name} regular containers have memory requests configured."
            )
            ...
            for container in (pod.containers or {}).values():
                ...
                if not mem:
                    report.status = "FAIL"
                    report.status_extended = f"Pod {pod.name} container {container.name} does not have a memory request configured."
                    break
