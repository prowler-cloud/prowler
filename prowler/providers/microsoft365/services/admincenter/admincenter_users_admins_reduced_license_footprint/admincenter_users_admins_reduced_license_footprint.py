...
class admincenter_users_admins_reduced_license_footprint(Check):
    def execute(self) -> Check_Report_Microsoft365:
        ...
        for tenant_domain, users in admincenter_client.users.items():
            for user_principal_name, user in users.items():
                admin_roles = [
                    role
                ...
                if admin_roles:
                    ...
                    report.status = "FAIL"
                    report.status_extended = f"User {user.name} has administrative roles {admin_roles} and license {user.license}."
