...
describe("client accordion content", () => {
  ...
  it("delegates data fetching to the hook instead of effect/ref choreography", () => {
    // The fetch (per-scan and cross-provider fan-out) lives in
    // ``useRequirementFindings`` — the component must not reintroduce inline
    // effect/ref choreography.
  ...
  it("detects cross-provider requirements from the scan/check augmentation maps", () => {
    // Cross-provider mode is detected by the presence of the
    // ``scan_ids_by_provider`` augmentation on the requirement. The reads
    // happen through a single ``xprov`` cast (see ``CrossProviderRequirement``)
    // and the maps are forwarded to the hook to drive the fan-out fetch.
