...
// Helper function to handle API responses consistently (Server-side version)
export const handleApiResponseWithRevalidation = async (
  response: Response,
...
// Helper function to handle API responses consistently (Client-safe version)
export const handleApiResponse = async (
  ...
  // Handle empty or no-content responses gracefully (e.g., 204, empty body)
  if (response.status === 204) {
    // Note: pathToRevalidate ignored in client-side version
    return { success: true, status: response.status } as any;
  }
