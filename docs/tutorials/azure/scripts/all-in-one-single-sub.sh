...
# All-in-one setup script for Prowler Azure authentication - Single Subscription Version
# This script handles:
# 1. Installation of Azure CLI (if needed)
# 2. Installation of jq (if needed)
# 3. Azure login (if needed)
# 4. Complete Prowler Azure authentication setup for the current subscription
#
# IMPORTANT NOTES:
...
# - You will need User Access Administrator or Owner role on the subscription for role assignments
...
# Banner
echo -e "${BLUE}${BOLD}=======================================================${NC}"
echo -e "${BLUE}${BOLD}   Prowler for Azure - Complete Setup (Single Sub)     ${NC}"
echo -e "${BLUE}${BOLD}=======================================================${NC}"
echo
...
# Now run the single subscription setup script
echo -e "${BLUE}${BOLD}Step 2: Setting up Prowler for Azure (Single Subscription)...${NC}"
echo
...
SINGLE_SUB_SCRIPT="single-subscription-setup.sh"

# Check if script exists in current directory
if [ -f "$SINGLE_SUB_SCRIPT" ]; then
    chmod +x "$SINGLE_SUB_SCRIPT"
    ./"$SINGLE_SUB_SCRIPT"
else
    # Try to find from current location
    SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
    if [ -f "$SCRIPT_DIR/$SINGLE_SUB_SCRIPT" ]; then
        echo -e "${GREEN}Found $SINGLE_SUB_SCRIPT in the same directory as this script.${NC}"
        chmod +x "$SCRIPT_DIR/$SINGLE_SUB_SCRIPT"
        "$SCRIPT_DIR/$SINGLE_SUB_SCRIPT"
    else
        # Try to find the script relative to this script
        SCRIPT_PATH=$(find . -name "$SINGLE_SUB_SCRIPT" -type f 2>/dev/null | head -n 1)
        
        if [ -z "$SCRIPT_PATH" ]; then
            # If not found locally, try searching in the repository
            SCRIPT_PATH=$(find / -path "*/prowler/docs/tutorials/azure/scripts/$SINGLE_SUB_SCRIPT" -type f 2>/dev/null | head -n 1)
        fi
        
        if [ -z "$SCRIPT_PATH" ]; then
            # As a last resort, download the script
            echo -e "${YELLOW}Could not find $SINGLE_SUB_SCRIPT locally. Downloading...${NC}"
            curl -s -o "$SINGLE_SUB_SCRIPT" "https://raw.githubusercontent.com/kourosh-forti-hands/prowler/master/docs/tutorials/azure/scripts/$SINGLE_SUB_SCRIPT"
