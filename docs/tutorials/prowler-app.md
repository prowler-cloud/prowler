...
Once a scan is complete, navigate to the Scan Jobs section to download the output files generated by Prowler. These outputs are bundled into a single .zip archive containing:
...
After the scan completes, you can also download individual compliance reports without downloading the full .zip archive.

To do this, go to the Compliance section, locate the desired report, and click its **Download button**. This will download the corresponding CSV file containing detailed report data.
