...
- **Processor Configuration**: Set up mute lists.
...
- Authentication to Prowler Cloud or Prowler App (self-managed) via Bearer token (API key or JWT token)
...
- Possibility to connect to a self-hosted Prowler App (e.g. self-hosted Prowler App)
- Authentication to Prowler App (self-managed) via Bearer token (API key or JWT token)
- Requires Python 3.12+ or Docker
