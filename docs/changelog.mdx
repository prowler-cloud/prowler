...
<Update label="v5.38.0" description="August 7, 2026">
  ...
  Read more in the [Compliance Watchlist documentation](/user-guide/compliance/tutorials/compliance#tracking-frameworks-with-the-compliance-watchlist).
  ...
  The Users table also shows each account's sign-in methods, including password, Google, GitHub, Partner SSO, and linked SAML domains.

  ![SAML configuration with multiple email domains](/images/prowler-app/saml/saml-multiple-domains.png)

  Read more in the [SAML SSO documentation](/user-guide/tutorials/prowler-app-sso#add-multiple-saml-domains).
  ...
  - AWS and GCP organization onboarding now launches scans for every linked provider through one bulk operation.
  - AWS delegated-administrator checks no longer report false failures when organization APIs cannot determine the administrator, and Security Hub responses now use the fields returned by the AWS API.
  - Imported scans now appear on the Scans page even when no provider is connected.
  - Secret-scanning checks no longer report credential-free JDBC connection strings as embedded credentials.
