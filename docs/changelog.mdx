...
<Update label="v5.41.0" description="September 2, 2026">
  ### 📥 Scans — Import Findings from the App
  ...
  Findings produced outside the platform, by the Prowler CLI or a CI pipeline, can now be brought into the app without leaving the browser. The Scans page gains an Import Findings dialog that takes a Prowler `.ocsf.json` report by drag-and-drop or file picker, hands it to the ingestion API and tracks the job to completion, reporting how many records were processed and how many were invalid. Files that are not an OCSF report, or are empty, are refused before any upload starts, and a rejected upload or a failed status poll can be retried in place. The dialog is available to roles holding the `manage_ingestions` permission.

  ![Import Findings button on the Scans page](/images/changelog/v5.41.0-import-findings-button.png)

  ![Import findings dialog with the drag-and-drop area](/images/changelog/v5.41.0-import-findings-dialog.png)
  ...
  Under the hood, the SDK's `Jira.send_finding()` now returns typed outcomes with the issue key, immutable ID and browse URL, and the new `Jira.get_issues_status()` resolves issues by immutable ID in batches, telling open, done, moved, missing and forbidden apart instead of treating every miss as a deletion.
  ...
  `security_login_challenges_configured` and `security_2sv_enforced` were unmapped from CIS Google Workspace 4.1.4.1 and CISA SCuBA `GWS.COMMONCONTROLS.1.1`, whose Post-SSO verification and phishing-resistant MFA requirements neither check can prove.

  Read more in the [Compliance documentation](https://docs.prowler.com/user-guide/compliance/tutorials/compliance).

  ### 🔍 Checks

  Ten new AWS checks land in this release, seven of them contributed by @tamg-aws. Thank you!
  ...
  - Checks no longer report `FAIL` when the scanning identity lacks permissions, an API is not enabled or a feature is not licensed. Twenty-eight Microsoft 365, Azure, AWS and GCP checks now emit a single `MANUAL` finding naming what is required, so a permissions gap is no longer indistinguishable from a real misconfiguration.
  - `ecr_registry_scan_images_on_push_enabled` no longer passes a registry whose scanning rules are all `MANUAL`, nor describes a `CONTINUOUS_SCAN` registry as scanning on push; each rule's `scanFrequency` is now read instead of inferred from the rule's presence.
  - `sagemaker_notebook_instance_without_direct_internet_access_configured` now reads the `DirectInternetAccess` setting instead of `RootAccess`, failing a notebook instance with direct internet access enabled even when root access is disabled.
  ...
  Read more in the [Image provider documentation](https://docs.prowler.com/user-guide/providers/image/getting-started-image).

  ### ☁️ AWS Provider — Partition Selection

  The new `PROWLER_AWS_PARTITION` environment variable selects the AWS partition (any partition known to botocore, such as `aws-cn` or `aws-us-gov`) used for STS credential validation and scan bootstrap, with a clear error when the account belongs to a different partition than the one requested.

  ### 🛠️ Prowler MCP Server — Failures Are Failures

  Prowler App tools now report a failure as an MCP tool execution error (`isError: true`, with the explanation in `content`) instead of a successful result carrying an `{"error": ...}` object, which clients and models read as a success. The Prowler Docs and Prowler Hub tools follow the same rule: `prowler_docs_search` no longer reports a failed search as zero matches, `prowler_docs_get_document` no longer reports a failed fetch as a missing page, and `prowler_hub_get_check_code` and `prowler_hub_get_check_fixer` now name the provider a check ID actually belongs to instead of reporting it as nonexistent. `prowler_get_compliance_framework_state_details` also rejects a call that passes both `scan_id` and `provider_id` instead of silently ignoring the provider.

  Read more in the [Prowler MCP documentation](https://docs.prowler.com/getting-started/products/prowler-mcp).

  ### 🧩 Prowler App

  - Creating a mute rule now updates only each affected provider's latest completed scan and future scans, so historical reaggregation no longer floods the Celery queues.
  - Cached permissions refresh from `/users/me?include=roles` after access token rotation, so a role change takes effect without signing out.
  - Slack integration polish: the connection check leaves its result on the card, naming the channel Slack refused or the channels it reached; a selected private channel carries the same `Private` badge as the channel list; and the bot is referred to as `@Prowler Cloud` throughout.
