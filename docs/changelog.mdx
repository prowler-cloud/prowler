...
<Update label="v5.37.0" description="July 31, 2026">
  ...
  ### 🧭 Cross-Provider Compliance — Across Accounts
  ...
  One framework, every account, a single answer. Building on the cross-provider-type roll-up from v5.34.0, the Compliance section now also aggregates a **single-provider framework** — CIS AWS, CIS GCP, ENS for Azure — across the latest completed scan of every account of that provider type. Each framework card rolls up into a consolidated posture with a per-account breakdown, a findings drill-down, and a combined executive PDF report. Requirement status follows the same strict precedence (FAIL over PASS over MANUAL), so one failing account flags the requirement for the whole estate.
  ...
  ![Cross-provider compliance detail across accounts](/images/compliance/prowler-app-across-providers-detail.png)
  ...
  Attack Paths adds four AWS privilege-escalation detection queries from [pathfinding.cloud](https://pathfinding.cloud), contributed by @paramanandmallik — thanks!
  ...
  Lighthouse AI now knows where you are in the app. Messages carry page-aware context — the page you are on, the finding or resource open in the side panel, and its metadata — so "explain this" just works, and each page offers concise contextual suggestions to start from.

  Lighthouse also gained access to every tool family the Prowler MCP server advertises: scan configurations, scan scheduling, finding triage, alert rules and recipients, integrations, users, and roles. Every action remains gated by RBAC: Lighthouse can only do what the user asking could do themselves.

  Read more in the [Lighthouse AI documentation](/getting-started/products/prowler-cloud-lighthouse).

  ### 🔌 Prowler MCP — New Tool Families

  Prowler MCP now manages integrations, users, and roles:

  - **Integrations** — list, create, update, and delete Amazon S3, AWS Security Hub, and Jira integrations, and send findings to Jira.
  - **Users** — read-only tools to list tenant users and identify the authenticated user.
  - **Roles** — browse RBAC roles and set the role a user holds.

  With a Prowler Cloud subscription, `prowler_cloud_*` tools also cover scan configurations, scan schedules, finding triage, and alerts (rules, recipients, and events).
  ...
  - HTML reports escape provider-originated finding fields, preventing stored cross-site scripting through malicious cloud resource tags.
  ...
  - SAML users without a `userType` attribute and without an existing role now receive a least-privilege `read_only` fallback role instead of elevated permissions.
