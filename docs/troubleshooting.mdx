...
This permission mismatch occurs due to the interaction between Docker volume ownership and container user privileges:

* The API container runs as user `prowler` with UID/GID 1000
* Docker creates the mounted volume directory `./_data/api` on the host with root ownership (UID 0)
* When the application attempts to write JWT key files (`jwt_private.pem` and `jwt_public.pem`), the operation fails due to insufficient permissions

**Solutions:**

There are two approaches to resolve this issue:

**Option 1: Adjust Volume Permissions (Recommended for Development)**

Before starting the containers, ensure the mounted volume directory has the correct ownership.

**On Linux with sudo access:**
...
**Cross-platform or without sudo (using Docker):**

```bash
docker run --rm -v $(pwd)/_data/api:/data alpine chown -R 1000:1000 /data
docker compose up -d
```

The Docker-based approach is more portable and works across Linux, macOS, and Windows without requiring elevated privileges on the host.

**Option 2: Use Environment Variables (Recommended for Production)**

Skip file-based key storage by providing JWT keys directly via environment variables. Generate RSA key pair and configure the `.env` file:
...
* Pre-creating the volume directory with appropriate ownership before the first run
* Using Docker named volumes instead of bind mounts (ownership is managed automatically by Docker)
* Implementing proper secrets management for production environments
