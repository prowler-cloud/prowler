...
For API development, Prowler provides a Makefile-based local stack in addition to the manual and Docker Compose workflows documented in the API README. This method runs PostgreSQL, Valkey, and Neo4j with Docker Compose, while Django and the Celery worker run natively through `uv`.

Before using this method, ensure `docker compose` and `tmux` are installed.
