...
<Tip>
New to adding a provider? Start with [Provider Golden Paths](./provider-golden-paths) to choose your scope (built-in CLI/SDK, external entry-point, or full-stack) before working through the full guide below.
</Tip>
---
...
Once you have decided the provider you want or need to add to Prowler, the next step is to study how to retrieve data from it. Based on that, the provider will fall into one of the following types: SDK, API, Tool/Wrapper or Hybrid, the last being a combination of the others (see **Special Case - Hybrid Providers** below, and the M365 and GitHub rows in the table that follows). New types may appear in the future, but these are the ones today.
...
<Note>
This step is now optional. `prowler/lib/cli/parser.py` keeps this hardcoded list for formatting, but any provider not in it is discovered automatically and appended to the CLI help text, using a `_cli_help_text` class attribute on your provider class. Editing this file gives your provider a spot in the primary list instead of an auto-appended one — it does not change whether the CLI recognizes it.
</Note>
...
<Note>
This step is still required — every current provider is wired into this `elif` chain. The base `Provider` class also exposes a `from_cli_args()` override point meant to let a provider handle its own argument parsing and instantiation instead of extending this chain, but as of this writing no built-in provider uses it, so treat it as an available option rather than the established pattern.
</Note>
...
<Note>
As of this writing, this `Provider` enum has no other references in the codebase — it is not required for a provider to work end to end. It may still be worth keeping in sync in case other tooling comes to depend on it, but don't treat a missing entry here as a functional gap without confirming a specific consumer needs it.
</Note>
...
<Note>
Optional, exactly as for SDK providers: a provider missing from this hardcoded list is still discovered and appended to the CLI help text via `_cli_help_text`. See the note on [Step 9 for SDK providers](#step-9-register-in-cli) for what editing the file does and does not change.
</Note>
...
<Note>
As with SDK providers, this `Provider` enum has no other references in the codebase as of this writing, so a missing entry is not a functional gap. See the note on [Step 12 for SDK providers](#step-12-add-to-config).
</Note>
...
TBD — this section is tracked as its own phase of [issue #12026](https://github.com/prowler-cloud/prowler/issues/12026). In the meantime, see [Scope 3](./provider-golden-paths#scope-3-full-stack-provider-covering-sdk-api-and-ui) in Provider Golden Paths for a pointer to using an existing full-stack provider as a working reference.
