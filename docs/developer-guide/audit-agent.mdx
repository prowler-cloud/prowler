...
The Audit Agent continuously audits **any GitHub repository** against **SOC 2** and **ISO 27001**. Register a target to get compliance findings on pull requests and on a schedule via the Prowler CLI and GitHub API.
...
| Credentials | `AUDIT_AGENT_TOKEN` secret (personal access token (PAT) or installation token with access to targets) |
...
<Note>
Nothing is committed to the target repository. Register a target in `targets.yml` and run a dry scan to validate the zero-touch flow.
</Note>
