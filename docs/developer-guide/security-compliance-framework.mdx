...
Prowler ships with 85+ compliance frameworks across AWS, Azure, GCP, Kubernetes, Microsoft 365, GitHub, and more. The catalog lives under `prowler/compliance/<provider>/`.
...
- Leave `Checks` as an empty array when the requirement cannot be automated. Manual requirements still appear in the report but always resolve to `MANUAL`.
