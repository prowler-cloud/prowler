...
By default, Prowler will audit a single StackIT project per scan. To configure it, you'll need to provide both a service account key file path and the project ID.
...
- **Key StackIT Responsibilities:**
    - Initializes and manages StackIT API authentication via a service account key file (the SDK mints and refreshes access tokens internally).
    - Validates the service account key path and project ID (UUID format validation).
...
Prowler reads the service account key file directly. The StackIT SDK signs the RSA challenge in the key and mints/refreshes short-lived access tokens internally, so there is no manual token rotation.
...
- **Service Account Key Path**:
  ...
  - The key file path is logged as-is; the secret material stays in the file on disk
