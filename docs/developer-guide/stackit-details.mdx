...
StackIT uses service account keys for API authentication. There are two authentication flows available:

#### Authentication Flow Comparison

| Feature | Key Flow (Recommended) | Token Flow (Deprecated) |
|---------|----------------------|------------------------|
| Security | High (short-lived tokens) | Lower (long-lived tokens) |
| Key Type | RSA key-pair based | Service account token |
| Status | Recommended | **Deprecated - Removed Dec 17, 2025** |

**Important**: The Token Flow is scheduled for deprecation and will be removed on **December 17, 2025**. Use the Key Flow for all new implementations.

### Creating a Service Account Key (Recommended Method)
...
#### Using Service Account Key File (Key Flow - Recommended)

```bash
# Set the path to your service account key file
export STACKIT_SERVICE_ACCOUNT_KEY_PATH="$HOME/.stackit/sa_key.json"
...
```bash
prowler stackit \
  --stackit-service-account-key-path "$HOME/.stackit/sa_key.json" \
  --stackit-project-id "12345678-1234-1234-1234-123456789abc"
```

#### Using Service Account Token (Token Flow - Deprecated)

**⚠️ Warning**: This method is deprecated and will be removed on December 17, 2025.
...
The StackIT SDK searches for credentials in the following order:

1. **Explicit configuration** in code (service account key path or token)
2. **Environment variables**:
   - `STACKIT_SERVICE_ACCOUNT_KEY_PATH` (Key Flow)
   - `STACKIT_SERVICE_ACCOUNT_TOKEN` (Token Flow - deprecated)
   - `STACKIT_PROJECT_ID`
...
```json
{
  "STACKIT_SERVICE_ACCOUNT_KEY_PATH": "/home/user/.stackit/sa_key.json",
