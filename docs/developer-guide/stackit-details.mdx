...
- **Key StackIT Models:**
    ...
    - `StackITRegionConfig`: Stores the region configuration (supports `eu01`, `eu02`).
...
### Generating Access Tokens from Service Account Keys

Prowler requires an access token for authentication, which can be generated from your service account key file using the StackIT CLI.

**Important**: Prowler does not directly use the service account key file (`.json`). You must first generate an access token from the key file.

#### Step 1: Install StackIT CLI
...
```bash
# Generate an access token from your service account key
stackit auth activate-service-account \
  --service-account-key-path ~/Downloads/sa-key-ae9ede46-2c81-4663-ad04-3dd992d6c565.json \
  --only-print-access-token
```

This will output a temporary access token that you can use with Prowler.

#### Step 3: Use the Access Token with Prowler

##### Option 1: Environment Variables

```bash
export STACKIT_API_TOKEN="<access-token-from-step-2>"
export STACKIT_PROJECT_ID="12345678-1234-1234-1234-123456789abc"

prowler stackit
```

**Note**: Access tokens are short-lived. If your scan fails with authentication errors, regenerate a new access token using the CLI command above.

### Credential Lookup Order

Prowler searches for credentials in the following order:
...
**Note**: The StackIT API token is only read from the environment.
...
**Network Interface (NIC) Filtering:**

All checks focus on security groups attached to NICs with public IP addresses. The implementation:

1. **Discovers all public IPs** in the project
2. **Identifies which NICs** have those public IPs assigned
3. **Flags security groups** attached to those public NICs as "in use"
4. **Reports only on "in use" security groups** (internet-exposed resources)

**Unattached NICs:**

NICs with public IPs are checked **even if not currently attached to a server**. This is intentional because:
- Unattached NICs with unrestricted security groups are "misconfigurations waiting to happen"
- If the NIC is attached to a server later, it's immediately exposed
- Defense-in-depth approach: flag potential issues before they become active threats
...
- **stackit-iaas** (v0.9.0+): IaaS service SDK for managing compute resources
- **stackit-resourcemanager** (v0.6.0+): Resource Manager SDK for fetching project metadata (e.g., project names)
...
```bash
export STACKIT_API_TOKEN="your-token"
...
2. **Service Coverage**: Only IaaS service is implemented (objectstorage service is work in progress)
...
**Error:** `StackIT API token is invalid or has expired`

**Solutions:**
1. Generate a new access token using: `stackit auth activate-service-account --service-account-key-path <path> --only-print-access-token`
2. Verify the service account key file is valid and not expired
3. Check that the service account has the necessary permissions (`iaas.viewer` or `project.owner`)
4. Ensure the token is properly set in environment variable or command-line argument
5. Note: Access tokens are short-lived and need to be regenerated periodically

**Error:** `StackIT credentials not found or are invalid`

**Solutions:**
1. Ensure both API token and project ID are provided
