...
`UI_AWS_QUICK_ONBOARDING_ENABLED` is a runtime experiment flag, off unless set to the exact string `"true"`. When it is on, picking **Amazon Web Services** in the provider wizard hands off to a separate two-step modal instead of the account-id form: the credentials (an IAM role ARN, with a collapsed CloudFormation quick-create helper, or static access keys) with an immediate connection test, then a name plus the first scan. The quick-create helper uses `permissions/templates/cloudformation/prowler-scan-role-quick.yml`, a single-parameter (`ExternalId`) template with the Prowler Cloud account fixed in the trust policy, published at `https://prowler-cloud-public.s3.eu-west-1.amazonaws.com/permissions/templates/aws/cloudformation/prowler-scan-role-quick.yml`. The classic wizard stays in place for every other provider and for AWS Organizations.
