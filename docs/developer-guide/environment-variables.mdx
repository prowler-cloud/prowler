...
`UI_AWS_QUICK_ONBOARDING_ENABLED` is a runtime experiment flag, off unless set to the exact string `"true"`. When it is on, choosing **Add A Single AWS Cloud Account** in the provider wizard opens a three-step flow instead of the account-id form: a CloudFormation quick-create button (or static access keys), the role ARN (the account id is read from it), and a name plus the first scan. The quick-create button uses `permissions/templates/cloudformation/prowler-scan-role-quick.yml`, a single-parameter (`ExternalId`) template with the Prowler Cloud account fixed in the trust policy, published at `prowler-cloud-public.s3.eu-west-1.amazonaws.com/permissions/templates/aws/cloudformation/prowler-scan-role-quick.yml`. The classic flow stays available for every other provider and for AWS Organizations.
