...
- Defaulting the attribute to `None` (data could not be read) instead of `[]`/`{}` (data was read and is empty), e.g. `logs_client.metric_filters is None` in the AWS CloudWatch checks.
...
Make sure the error branch only captures real access errors. A `404`/not-found response frequently means the feature is simply not configured, which **is** a legitimate `FAIL`; a `403`, an "API not enabled" error or an unexpected exception is not.
