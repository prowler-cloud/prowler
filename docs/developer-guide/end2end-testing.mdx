...
- **Ensure Prowler API is available**
  - By default, Playwright uses `WEB_APP_API_BASE_URL=http://localhost:8080/api/v1` (configured in `playwright.config.ts`).
  ...
  - If a different API URL is required, set `WEB_APP_API_BASE_URL` accordingly before running the tests.
