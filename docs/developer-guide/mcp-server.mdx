...
- [Configuration](/getting-started/basic-usage/prowler-mcp) - Configure Claude Desktop, Cursor, and other MCP clients
- [Tools Reference](/getting-started/basic-usage/prowler-mcp-tools) - Complete list of all 34 available tools
</Info>
...
| Sub-Server | Tool Count | Auth Required | Description |
|------------|------------|---------------|-------------|
| Prowler Hub | 10 tools | No | Security checks catalog with **over 1000 checks**, fixers, and **70+ compliance frameworks** |
| Prowler App | 22 tools | Yes | Full access to Prowler Cloud and Self-Managed features |
| Prowler Documentation | 2 tools | No | Full-text search and retrieval of official documentation |
...
├── prowler_hub/              # 10 tools (no auth)
├── prowler_app/              # 22 tools (auth required)
│   ├── tools/                # Tool implementations
│   ├── models/               # Pydantic models
│   └── utils/                # API client, auth, loader
└── prowler_documentation/    # 2 tools (no auth)
```
...
2. **Auto-Discovery** (Prowler App): Tools extending `BaseTool` are automatically discovered and registered

## Adding Tools to Prowler App

Prowler App tools use the `BaseTool` abstract class with automatic registration. This pattern supports the key capabilities documented in the [Overview](/getting-started/products/prowler-mcp#key-capabilities):
- Provider Management
- Scan Orchestration
- Findings Analysis
...
Tool docstrings become LLM descriptions. Write them to support the [use cases](/getting-started/products/prowler-mcp#use-cases) documented in the Overview:
...
Use Pydantic `Field()` with clear descriptions:
...
- [Installation Guide](/getting-started/installation/prowler-mcp) - Docker, PyPI, and source installation
...
<CardGroup cols={2}>
  ...
  <Card title="Tools Reference" icon="wrench" href="/getting-started/basic-usage/prowler-mcp-tools">
    Complete reference of all 34 available tools
  </Card>
  ...
  <Card title="Lighthouse AI" icon="robot" href="/getting-started/products/prowler-lighthouse-ai">
    AI-powered security analyst using similar architecture
  </Card>
</CardGroup>
