...
### Configurable Checks
The following list includes all the Fly.io checks with configurable variables that can be changed in the configuration YAML file:

| Check Name                            | Value                      | Type             | Default                                                                                   |
|---------------------------------------|----------------------------|------------------|-------------------------------------------------------------------------------------------|
| `app_no_public_ip_address`            | `public_apps`              | List of Strings  | `[]`                                                                                      |
| `machine_no_public_non_http_ports`    | `allowed_public_ports`     | List of Integers | `[80, 443]`                                                                               |
| `machine_no_plaintext_secrets_in_env` | `secret_env_name_patterns` | List of Strings  | `["PASSWORD", "SECRET", "TOKEN", "API_KEY", "PRIVATE_KEY", "CREDENTIAL", "PASSPHRASE"]` |

### Provider Settings

| Value         | Type    | Default | Description                                                                                    |
|---------------|---------|---------|------------------------------------------------------------------------------------------------|
| `max_retries` | Integer | `3`     | Retries during a scan for Machines API and GraphQL HTTP 429 responses, honoring `Retry-After`, and for Machines API connection failures. GraphQL connection failures and HTTP 5xx responses are not retried. Range `0..10`; `0` disables retries. |

Rate-limit waits are capped at 300 seconds each and 600 seconds per request in total. Organization discovery and credential validation happen before the scan and are not retried.

<Note>
Each entry of `allowed_public_ports` must be a port between `1` and `65535`; an invalid list is ignored and the default is used. A key set to `null` also falls back to its default, while an explicit empty list (for example `allowed_public_ports: []`) is applied as configured.
</Note>
...
fly:
  ...
  # Apps that are expected to serve public traffic and may hold a public IP
  # address. Every other app is expected to be private-only.
  public_apps: []
  # fly.machine_no_public_non_http_ports
  # Edge ports a machine service may publish. Defaults to HTTP/HTTPS only.
