...
Some AWS services accumulate large numbers of resources (EBS snapshots, backup recovery points, CloudWatch log groups, Lambda functions, ECS task definitions, and CodeArtifact packages). Scanning every resource increases scan time, cost, and API throttling, and produces noisy findings. Prowler limits the number of resources scanned per service, defaulting to the latest 100, and always prioritizes `FAIL` findings so non-compliant resources are never hidden before compliant ones up to the limit. Where the AWS API supports server-side ordering, the most recent resources are scanned first; otherwise the order is best-effort.

The global default applies to every supported service and is overridable per service. Set a value to `0` (or a negative number) to disable the limit and scan every resource.

| Value                              | Scope                                                   | Type    |
|------------------------------------|---------------------------------------------------------|---------|
| `max_scanned_resources_per_service`| Global default for all supported services (default 100) | Integer |
...
aws:
  ...
  # Limits the number of resources scanned per service, prioritizing FAIL
  # findings. Set to 0 (or a negative value) to disable the limit.
