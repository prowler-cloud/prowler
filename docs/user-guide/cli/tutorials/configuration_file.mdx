...
Some AWS services accumulate large numbers of resources (EBS snapshots, backup recovery points, CloudWatch log groups, Lambda functions, ECS task definitions, and CodeArtifact packages). Scanning every resource increases scan time, cost, API throttling, and finding volume. Prowler limits how many resources are analyzed for these high-volume AWS resource paths, defaulting to the latest 100 where AWS API ordering or resource timestamps support it. Otherwise, Prowler uses best-effort API order.

The global default applies to the supported resources below and is overridable per resource. Set a value to `0` (or a negative number) to disable the limit and scan every resource. The limit applies to resources selected for analysis, not to findings; a selected resource may produce zero, one, or many findings.
...
| Value                              | Scope                                                   | Type    |
|------------------------------------|---------------------------------------------------------|---------|
| `max_scanned_resources_per_service`| Global default for supported high-volume AWS resources (default 100) | Integer |
...
aws:
  ...
  # Limits the number of selected high-volume AWS resources analyzed.
