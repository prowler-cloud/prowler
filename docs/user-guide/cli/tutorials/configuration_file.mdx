...
|---------------|---------|---------|------------------------------------------------------------------------------------------------|
| `max_retries` | Integer | `3`     | Retries for Machines API requests that are rate limited (HTTP 429, honoring `Retry-After`) or fail at the connection level (timeouts, connection errors); HTTP 5xx responses are not retried. Range `0..10`; `0` disables retries. |
