...
Prowler for Linode uses a **Personal Access Token** (PAT) for authentication. This is the only authentication method currently supported.
...
| `linodes` | `Read Only` | Required to list instances, their configurations, and firewalls |
...
Then run Prowler without the token flag:
...
<Warning>
Passing secrets directly on the command line may expose them in shell history and process listings. The environment variable method is strongly recommended for production and CI/CD use.
</Warning>
