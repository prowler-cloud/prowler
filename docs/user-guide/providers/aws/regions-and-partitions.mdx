...
You can also configure the exclusion list with the `PROWLER_DISALLOWED_REGIONS` environment variable:

```console
export PROWLER_DISALLOWED_REGIONS="eu-west-1 me-south-1"
prowler aws
```
...
1. `--excluded-region`
2. `PROWLER_DISALLOWED_REGIONS`
3. `aws.disallowed_regions` in `config.yaml`

<Note>
For self-hosted App or API-triggered scans, set `PROWLER_DISALLOWED_REGIONS` in the runtime environment of the backend scan containers such as `api` and `worker`. The `ui` container does not enforce AWS region selection.
