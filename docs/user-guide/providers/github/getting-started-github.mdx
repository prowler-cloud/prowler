...
To scan only specific repositories, use the `--repository` flag with repository names in `owner/repo-name` format:

```console
prowler github --repository owner/repo-name owner/another-repo
```

#### Scan Specific Organizations

To scan all repositories belonging to one or more organizations or users, use the `--organization` flag:

```console
prowler github --organization my-org another-org
```
