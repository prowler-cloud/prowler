...
This guide walks you through setting up GitHub security scanning with Prowler. Choose your preferred interface below:
...
We strongly recommend using **Fine-Grained Personal Access Tokens** because they provide:
...
<Warning>
**Critical: User Scans vs Organization Scans**

The scan scope depends entirely on how you invoke Prowler CLI:

| Command | What Gets Scanned | Organization Checks Included? |
|---------|------------------|-------------------------------|
| `prowler github` | All repositories the token has access to |
| `prowler github --repository owner/repo` | Single specified repository |
| `prowler github --organization org-name` | Organization repos + org settings |
| `prowler github --organization org-name --repository owner/repo` | Organization + single repository |

**Key Points:**
- Scanning **user repositories** does NOT run organization-level checks
- To audit organization MFA, security policies, etc., you MUST use `--organization`
- If you're a member of multiple organizations, specify each one explicitly
...
- [Prowler CLI Reference](/user-guide/cli-reference)
