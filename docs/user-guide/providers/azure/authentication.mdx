...
3. Generate a certificate (see [Certificate Authentication](#certificate-authentication) below) and paste the base64-encoded public certificate and its SHA-1 thumbprint into the Bicep parameters.
4. Deploy and, when the stack finishes, copy the `tenantId`, `applicationId`, and `certificateThumbprint` outputs plus the private key back into Prowler.
...
# SHA-1 thumbprint — feeds `certificateThumbprint` in the Bicep template
openssl x509 -in prowler.crt -noout -fingerprint -sha1 \
    | cut -d= -f2 | tr -d ':' | tr '[:lower:]' '[:upper:]'

# Base64 of the private key — paste into Prowler as "Certificate Content"
base64 -i prowler.key | tr -d '\n'
```
...
- **Certificate Content** — the base64-encoded **private key** (never the public certificate).
