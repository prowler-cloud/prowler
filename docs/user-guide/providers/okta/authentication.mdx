...
On the app, open the **Okta API Scopes** tab and click **Grant** on every scope Prowler needs. The bundled checks require `okta.policies.read`, `okta.brands.read`, `okta.apps.read`, and `okta.networkZones.read`.
...
# Optional — defaults to "okta.policies.read,okta.brands.read,okta.apps.read,okta.networkZones.read"
export OKTA_SCOPES="okta.policies.read,okta.brands.read,okta.apps.read,okta.networkZones.read"
...
- **`invalid_scope`** — one of the requested scopes (`okta.policies.read`, `okta.brands.read`, `okta.apps.read`, and `okta.networkZones.read`) is not granted on the service app. Grant the missing scope from **Okta API Scopes**.
