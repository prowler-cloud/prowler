...
Prowler requests a fixed set of OAuth scopes on every token exchange. The defaults cover every bundled check across the Sign-On, Application, and Authenticator services:
...
prowler okta --okta-scopes okta.policies.read okta.brands.read okta.apps.read okta.authenticators.read
```
