...
| Region | `HUAWEICLOUD_REGION` (or `HW_REGION`) | Default region when `--region` is not passed (e.g. `eu-west-101`) |
...
<Warning>
Huawei Cloud runs separate clouds: **International** and **China** (`.com` endpoints) and **Huawei Cloud Europe** (`.eu` endpoints). The region you select determines the endpoint, so accounts outside China must pass a region they can reach — for example `eu-west-101` for a Huawei Cloud Europe account. Credentials from one cloud do not work against another.
</Warning>
