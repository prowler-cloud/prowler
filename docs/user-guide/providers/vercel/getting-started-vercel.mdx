...
Prowler currently includes 26 Vercel checks. Only the 7 checks below have explicit billing-plan handling in the provider metadata or check logic. When the scanned scope reports a billing plan, Prowler adds plan-aware context to findings for these checks. If the API does not expose the required configuration, Prowler may return `MANUAL` and require verification in the Vercel dashboard.
...
<Note>
`security_waf_enabled` and `security_managed_rulesets_enabled` return `MANUAL` when the firewall configuration endpoint is not accessible from the API. The other 19 current Vercel checks do not currently include plan-specific handling in the provider. As long as the API token can read the required resources, those checks can be assessed regardless of plan.
</Note>
