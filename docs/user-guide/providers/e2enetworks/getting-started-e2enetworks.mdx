...
Once authenticated, export the credentials as environment variables and run Prowler for E2E Networks. Prowler reads the secrets exclusively from environment variables, so they are never exposed in shell history or process listings:
...
```bash
prowler e2enetworks --checks node_encryption_enabled storage_bucket_public_access_disabled
```
...
Use `--e2e-networks-location` (alias `--e2e-networks-locations`) to limit the scan to one or more E2E Networks regions. When the flag is omitted, Prowler reads `E2E_NETWORKS_LOCATION` and falls back to `Delhi`.

```bash
prowler e2enetworks --e2e-networks-location Delhi Mumbai
```
...
| `storage` | Object storage buckets and block volumes with their protection settings |
