...
Prowler for E2E Networks authenticates against the E2E Networks MyAccount API using an **API key**, an **auth token**, and a numeric **project ID**. Prowler reads the API key and auth token **exclusively** from environment variables, so the secrets are never exposed in shell history or process listings.
...
| Location | `E2E_NETWORKS_LOCATION` | Optional region to scan (defaults to `Delhi`) |
...
```bash
prowler e2enetworks --e2e-networks-location Delhi
```
