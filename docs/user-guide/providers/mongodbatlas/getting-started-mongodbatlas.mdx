...
Prowler now supports MongoDB Atlas both from the CLI and from **Prowler App**. This guide walks you through the requirements, how to connect the provider in the UI, and how to run scans from the command line.
...
3. An **API Key pair** (public and private keys) with _Organization Read Only_ or any role that can list projects/clusters. If you want to run checks for Auditing configuration it's also necessary to add _Organization Owner_ permission.
