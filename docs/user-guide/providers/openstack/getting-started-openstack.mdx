...
1. Navigate to **Cloud Providers** and click **Add Cloud Provider**.
   {/* <!-- Screenshot: Add provider list showing OpenStack option --> */}
2. Select **OpenStack** from the provider list.
3. Enter your **Project ID** from your OpenStack provider.
   {/* <!-- Screenshot: Add project ID form --> */}
4. (Optional) Add a friendly alias to identify this project in dashboards.

### Step 2: Provide credentials

1. Click **Next** to open the credentials form.
2. Paste the full content of your `clouds.yaml` file into the **Clouds YAML Content** field. You can download this file from your OpenStack provider's Horizon dashboard (see the [Authentication guide](/user-guide/providers/openstack/authentication) for detailed instructions).
3. Enter the **Cloud Name** — this is the key that identifies your cloud entry inside the `clouds.yaml` file (e.g., `mycloud`).
   {/* <!-- Screenshot: Credentials form with clouds.yaml content and cloud name fields --> */}
...
2. The provider will appear in the list with its current connection status.
   {/* <!-- Screenshot: Provider list showing OpenStack with connection status --> */}
3. Launch a scan from the provider row or from the **Scans** page.
   {/* <!-- Screenshot: Launch scan button/page --> */}
