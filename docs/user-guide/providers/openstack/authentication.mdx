...
<Tabs>
  <Tab title="OVH">
    ...
    2. Click on **Horizon** in the left sidebar (or access the Horizon URL provided by OVH)
    3. Log in with the user credentials created in Step 1 (ensure to enter with that user because if you enter with the root user you will download openstack credentials for the root user, if the user is not the correct one, log out and log in again)
  ...
  <Tab title="Generic Public Cloud">
    ...
    <Note>
    Consult your provider's documentation for specific instructions on creating users and assigning roles. Common providers include:
    - **Infomaniak**: Public Cloud management interface
    - **Vexxhost**: Account dashboard
    - **Fuga Cloud**: Account portal
    - **Cloudandheat**: OpenStack dashboard
    </Note>
...
<Warning>
Using Public Cloud credentials can limited the KeyStone API access so maybe the command above will not work and you will need to verify the permissions in the provider's control panel.
</Warning>
