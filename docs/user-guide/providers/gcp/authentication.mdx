...
- **Custom `ProwlerRole` (recommended)** – Include granular permissions that might be missing from the basic Reader role. At minimum, add `storage.buckets.getIamPolicy` so Prowler can inspect Cloud Storage IAM policies when evaluating relevant checks.
