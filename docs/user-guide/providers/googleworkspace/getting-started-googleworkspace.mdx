...
Prowler for Google Workspace audits your organization's Google Workspace environment for security misconfigurations, including super administrator account hygiene, domain settings, and more.
...
- Create a Service Account in a GCP project with Domain-Wide Delegation enabled
- Authorize the required read-only OAuth scopes in the Google Workspace Admin Console
- Identify the Google Workspace **Customer ID** to use as the provider identifier
- Have the email of a **super administrator** to use as the delegated user
...
3. Enter the **Delegated User Email** (a super administrator in your Google Workspace organization).
...
Prowler authenticates as the delegated user and runs all available security checks against your Google Workspace organization.
