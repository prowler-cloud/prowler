...
<Note>
Cross-Provider Compliance aggregates results across multiple provider scans, so it lives in the Compliance section of Prowler Cloud and Prowler Enterprise. It is not part of Prowler CLI. In the free tier the **Cross-provider** tab is visible but disabled, while the per-scan [Compliance](/user-guide/compliance/tutorials/compliance) view remains fully available.
</Note>

## How Cross-Provider Compliance Works

Some Prowler frameworks are **universal**: a single JSON that declares its requirements once and maps each requirement to checks for many providers at the same time (for example, the same CSA CCM control mapped to both AWS and Azure checks). These frameworks live at the top of the compliance catalog (`prowler/compliance/<framework>.json`), as opposed to the legacy per-provider frameworks under `prowler/compliance/<provider>/`.

Cross-Provider Compliance leverages this structure to answer a single question: **"How compliant is my whole estate against this framework, regardless of provider?"** For a chosen universal framework, Prowler Cloud:
...
<Note>
Filters narrow **which providers contribute** to the aggregation. They do not change how a requirement rolls up — only providers that actually contributed a result for a requirement are counted toward its status.
</Note>
...
* **Provider Coverage:** Shows which compatible providers contributed a scan and their individual posture, making coverage gaps and per-provider weak spots visible at a glance.
...
Cross-Provider Compliance rolls up results in two stages, following a strict **FAIL > PASS > MANUAL** precedence.
...
A report is not generated for the whole framework in the abstract: it is generated for the **exact set of scans your current filters resolve to**. The provider type, provider / account, and provider group filters applied on the detail page determine which providers contribute, and the auto-select rule pins each contributing provider's latest completed scan. That resolved scan set — together with the framework — is what the PDF is built from.
...
The Cross-Provider Compliance view is backed by dedicated Prowler API endpoints. The roll-up and PDF actions are reachable programmatically to integrate cross-provider evidence into dashboards, ticketing systems, or downstream pipelines:

* **Retrieve the cross-provider roll-up:** `GET /api/v1/cross-provider-compliance-overviews?filter[compliance_id]=<framework_id>`
* **Generate the combined PDF report:** `POST /api/v1/cross-provider-compliance-overviews/pdf?filter[compliance_id]=<framework_id>` — returns a task; poll `GET /api/v1/tasks/{id}` until it completes.
* **Download the generated PDF:** `GET /api/v1/cross-provider-compliance-overviews/pdf/{task_id}`
* **Check for an existing report:** `GET /api/v1/cross-provider-compliance-overviews/pdf/latest?filter[compliance_id]=<framework_id>`

All endpoints accept the same `filter[provider_type__in]`, `filter[provider_id__in]`, and `filter[provider_groups__in]` filters as the UI, and resolve the contributing scans with the same auto-select rule and role-based visibility. Read-only actions require no additional permission beyond provider visibility; generating a report requires the `MANAGE_SCANS` permission. See the [Prowler API reference](https://api.prowler.com/api/v1/docs) for the full schema.
