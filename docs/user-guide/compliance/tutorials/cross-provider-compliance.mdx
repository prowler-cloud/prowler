...
<Note>
The **Across provider types** section requires at least one completed scan for a provider compatible with a universal framework. If none is available, that section shows a notice prompting you to launch or wait for a scan to complete. The **Across providers** section below it has its own requirements — see [Cross-Account Compliance](/user-guide/compliance/tutorials/cross-account-compliance).
</Note>
...
* **Across providers:** One card per single-provider framework (for example, CIS AWS) that can be aggregated across every provider of the same type. See [Cross-Account Compliance](/user-guide/compliance/tutorials/cross-account-compliance).
...
* **Providers:** Narrow to specific providers — accounts, subscriptions, or projects — listed by alias and unique identifier (UID).
...
The aggregation works the same way, with the column axis swapped from provider type to individual provider, and it produces its own combined PDF report. See [Cross-Account Compliance](/user-guide/compliance/tutorials/cross-account-compliance) for the full guide.

## Related Documentation

* [Cross-Account Compliance](/user-guide/compliance/tutorials/cross-account-compliance)
