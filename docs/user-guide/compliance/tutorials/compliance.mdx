...
* **Multiple Scans:** Aggregates one framework across several scans at once, either across provider types ([Cross-Provider Compliance](/user-guide/compliance/tutorials/cross-provider-compliance)) or across every provider of the same type ([Cross-Account Compliance](/user-guide/compliance/tutorials/cross-account-compliance)). It is the landing tab in Prowler Cloud; in Prowler Local Server it carries a **Cloud** badge, since it requires a subscription.
