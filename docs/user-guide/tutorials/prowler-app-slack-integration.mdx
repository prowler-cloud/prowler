...
The Slack integration is available in **Prowler Cloud** only. Prowler Local Server does not serve the Slack endpoints at all, so the Slack card does not appear on the Integrations page and the management page redirects away.
