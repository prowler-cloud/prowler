...
<Note>
The page lives under the Scans area (`/scans/config`), mirroring how Lighthouse AI keeps its settings at `/lighthouse/config`. The sidebar item is labeled **Scan** because it already sits inside the **Configuration** group.
</Note>
