...
1. Click the link above or use the **Create Stack in Management Account** button in the Prowler wizard (which also pre-fills the ExternalId).
...
{/* TODO: replace screenshot — wizard now shows External ID first, then two deployment buttons (Create Stack + Open StackSets Console), then Role ARN */}
<Frame>
...
{/* TODO: replace screenshot — field label is now "Management Account Role ARN" */}
<Frame>
