...
<Note>
This feature is **exclusively available in Prowler Cloud**. It is not available in the self-hosted Prowler App or the Prowler CLI. For CLI-based multi-account scanning, see [AWS Organizations in Prowler CLI](/user-guide/providers/aws/organizations).
</Note>
...
Before using the Prowler wizard, you need to deploy **two IAM roles** in your AWS environment. The onboarding follows this sequence:

<Frame caption="Steps 1 and 2 are done once in AWS. Steps 3 and 4 are done in Prowler Cloud.">
  <img src="/images/organizations/onboarding-flow.png" alt="Onboarding flow: 1. Create Management Account Role, 2. Deploy StackSet, 3. Run the Wizard, 4. Launch Scans" />
</Frame>
...
|------|---------------|--------------|------------------|
| **ProwlerScan** (management account) | Your management (root) account only | Discovers the Organization structure **and** scans the management account. Has additional `organizations:*` permissions. | **Manually** in the IAM Console ([Step 1](#step-1-create-the-management-account-role)). Cannot be deployed via StackSet. |
...
<Frame caption="Both roles share the same name (ProwlerScan). The management account role includes additional Organization discovery permissions.">
  <img src="/images/organizations/two-roles-architecture.png" alt="Two Roles Architecture: ProwlerScan in management account (discovery + scanning) and ProwlerScan in member accounts (scanning only)" />
</Frame>

<Note>
**Same name, different permissions.** Both roles are named `ProwlerScan` — Prowler expects a consistent role name across all accounts. The management account role has the same scanning permissions as member accounts (`SecurityAudit` + `ViewOnlyAccess`), plus additional `organizations:*` permissions for Organization discovery.
</Note>
...
If your AWS accounts use **private subnets**, ensure that outbound internet access is available through one of the following:

- A **NAT Gateway** or **NAT Instance** in the VPC.
- **VPC Endpoints** for the AWS services that Prowler needs to reach (STS, Organizations, and the services being scanned).

Without outbound connectivity, Prowler Cloud will not be able to assume roles in your accounts or run scans.
...
1. Review the pre-filled parameters:
   ...
   - **ExternalId parameter**: Pre-filled with your tenant's External ID.
...
- **Least privilege**: The ProwlerScan role deployed by the StackSet uses `SecurityAudit` and `ViewOnlyAccess` — AWS managed policies that grant read-only access. These are the minimum permissions Prowler needs to assess your accounts without making any changes.
...
- Check the role has all `organizations:*` permissions listed in [Step 1](#step-1-create-the-management-account-role)
