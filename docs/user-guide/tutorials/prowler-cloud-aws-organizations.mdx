...
The Organizations wizard uses the same schedule controls described in [Scan Scheduling](/user-guide/tutorials/prowler-scan-scheduling).

Available options depend on the account type:

| Account Type | Available Action |
|--------------|------------------|
| Prowler Cloud paid subscription | Create a daily, every 48 hours, weekly, or monthly recurring schedule for all connected accounts. |
| Prowler Cloud trial or onboarding | Run one-time manual scans only. |
| Prowler App or non-Cloud deployments | Create a daily recurring scan or run a one-time manual scan. |

For paid Prowler Cloud accounts, select **Launch an initial scan now for immediate findings** to run a scan immediately after the recurring schedule is saved.

### Launch

Click **Save**, **Save and launch scan**, or **Launch scan**, depending on the selected schedule option. A toast notification confirms whether the schedule was saved, scans were launched, or both. The toast includes a link to the **Scans** page. You will be redirected to the **Providers** page.
