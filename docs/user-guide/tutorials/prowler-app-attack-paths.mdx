...
Use `EXISTS { ... }` to express "at least one item in the list satisfies a predicate". The `EXISTS` subquery returns `true` as soon as a matching child is found, which keeps the traversal short on parents with many items.

To find statements that grant any S3 read action:
...
To check whether every item in the list satisfies a predicate, combine `EXISTS` for the positive case with a negated `EXISTS` for the counter-example:

```cypher
MATCH (stmt:AWSPolicyStatement)
WHERE EXISTS {
    MATCH (stmt)-[:HAS_ACTION]->(:AWSPolicyStatementActionItem)
  }
  AND NOT EXISTS {
    MATCH (stmt)-[:HAS_ACTION]->(a:AWSPolicyStatementActionItem)
...
For the "is any item of this list a substring of a dynamic value" case, such as "does any resource pattern in this policy match a target role ARN", bind the item value in the subquery and apply the substring check against it:
