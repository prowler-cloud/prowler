...
WHERE stmt.effect = 'Allow'
  AND size([action IN split(stmt.action, ',') WHERE action = '*']) > 0
...
Some Cartography properties represent lists of values, such as `action`, `resource`, `notaction`, and `notresource` on `AWSPolicyStatement` nodes. In the Attack Paths graph, list-typed properties are stored as comma-delimited strings to keep the schema portable across graph backends. Read them back with `split(prop, ',')`.

**Membership** uses the standard `IN` operator on the split result:

```cypher
MATCH (stmt:AWSPolicyStatement)
WHERE 'iam:DeletePolicy' IN split(stmt.action, ',')
RETURN stmt
LIMIT 25
```

**Predicate filters** (asking whether any, all, or none of the elements satisfy a condition) are expressed with a list comprehension wrapped in `size(...)`:

| Intent | Pattern |
|---|---|
| At least one element matches | `size([x IN list WHERE pred]) > 0` |
| No element matches | `size([x IN list WHERE pred]) = 0` |
| Every element matches | `size([x IN list WHERE pred]) = size(list)` |

Example, finding statements that grant any S3 action:

```cypher
MATCH (stmt:AWSPolicyStatement)
WHERE size([action IN split(stmt.action, ',') WHERE action STARTS WITH 's3:']) > 0
RETURN stmt
LIMIT 25
```

The "every element matches" form repeats the `split(...)` call on both sides of the comparison. When that becomes noisy, hoist the split into a `WITH` clause first:
