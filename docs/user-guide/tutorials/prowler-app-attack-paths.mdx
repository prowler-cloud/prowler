...
Some Cartography properties represent lists of values, such as `action`, `resource`, `notaction`, and `notresource` on `AWSPolicyStatement` nodes. The Attack Paths graph stores list-typed properties as comma-delimited strings to keep the schema portable across graph backends. Two patterns read them back: string operators applied directly to the comma-joined value, and `split(prop, ',')` for the cases that genuinely need to iterate the individual tokens.

**Prefer string operators for the common shapes.** `=`, `STARTS WITH`, `ENDS WITH`, and `CONTAINS` run as filters over the raw stored value without materialising a list, which keeps response times low on large graphs. The table below maps each predicate intent to a `split`-free form. Let `P` be the comma-string property and `L` the literal needle.

| Intent (per token) | Form on `P` |
|---|---|
...
| Some token starts with `'pre'` | `toLower(P) STARTS WITH 'pre' OR toLower(P) CONTAINS ',pre'` |
| Some token contains `'sub'` | `toLower(P) CONTAINS 'sub'` |
| Some token contains a dynamic value | `P CONTAINS <expr>` |

Example, finding statements that grant any S3 read action:

```cypher
MATCH (stmt:AWSPolicyStatement)
WHERE stmt.effect = 'Allow'
  AND (
    toLower(stmt.action) STARTS WITH 's3:get'
    OR toLower(stmt.action) CONTAINS ',s3:get'
    OR toLower(stmt.action) STARTS WITH 's3:list'
    OR toLower(stmt.action) CONTAINS ',s3:list'
  )
RETURN stmt
LIMIT 25
```

**Reach for `split` only when the intent cannot be expressed with string operators.** Two shapes need it.

The "every token must match" case has no string-op equivalent. Hoist the split into a `WITH` clause to avoid evaluating it twice:

```cypher
MATCH (stmt:AWSPolicyStatement)
WITH stmt, split(stmt.action, ',') AS actions
WHERE size([action IN actions WHERE action STARTS WITH 's3:']) = size(actions)
RETURN stmt
LIMIT 25
```

The "a token of `P` is a substring of a dynamic value" case, such as "is any resource pattern in this policy a substring of this role's ARN", needs the loop variable as the needle. Put the cheap string clauses first so the `split` runs only when none of them matched:

```cypher
MATCH (role:AWSRole), (stmt:AWSPolicyStatement)
WHERE stmt.effect = 'Allow'
  AND (
    stmt.resource = '*'
...
Cypher evaluates `OR` left to right, so the `split` subquery runs only for resources that did not match any of the cheap clauses.

Returning the raw value (`RETURN stmt.action`) is also valid when the comma-delimited form is acceptable in the output.
...
- Prefer string operators (`=`, `STARTS WITH`, `ENDS WITH`, `CONTAINS`) over `split(prop, ',')` when checking comma-string properties such as `action` or `resource`. See [Working with List-Typed Properties](#working-with-list-typed-properties) for the per-shape recommendations.
...
- **List-typed properties** such as `action` or `resource` on `AWSPolicyStatement` are stored as comma-delimited strings. See [Working with List-Typed Properties](#working-with-list-typed-properties) for the read pattern.
