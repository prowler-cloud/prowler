...
<Note>
  In Prowler Cloud and Prowler Private Cloud, the query selector hides queries confirmed empty for the selected scan, so only queries that return data remain visible. See [Active Queries](/user-guide/tutorials/prowler-app-attack-paths-active-queries).
</Note>
