...
<Note>
Scan scheduling availability depends on the account type:

* **Prowler Cloud paid subscription:** Custom recurring schedules.
* **Prowler Cloud trial or onboarding:** Manual scans only.
* **Prowler App and other open-source deployments:** Daily recurring scans every 24 hours, tied to the time when the provider was created.
</Note>
...
* Custom recurring schedules require a Prowler Cloud paid subscription.

## Schedule Options

Prowler Cloud accounts with a paid subscription support these custom recurring schedule options:
...
The schedule time uses the browser timezone when the schedule is saved. Prowler displays the next scheduled scan in that timezone.
...
|--------------|------------------------|
| Prowler Cloud paid subscription | Run scans now, create custom recurring schedules, edit schedules, remove schedules, and optionally launch an initial scan after saving. |
...
| Prowler App or other open-source deployments | Run manual scans and create daily recurring schedules every 24 hours during provider launch. The daily schedule is tied to the provider creation time, and custom cadence controls are unavailable. |
