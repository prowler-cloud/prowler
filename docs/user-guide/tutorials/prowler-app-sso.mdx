...
<Tabs>
    ...
    <Tab title="Okta App Catalog">
        ...
        7. **Configure User Attributes in Okta**: Okta acts as the central source for user profile information. Prowler Cloud maps the following Okta user profile attributes during each SAML login:
            ...
            * If `userType` is not defined in the user's Okta profile, the user's existing roles in Prowler Cloud are left unchanged. Users without an existing role receive the `read_only` role until a Prowler administrator assigns another role.
