...
Prowler Cloud onboards every Azure subscription in your Microsoft Entra tenant through a single guided wizard. Instead of connecting subscriptions one by one, you can discover every Management Group and subscription under the tenant root Management Group, select the ones you want to monitor, test connectivity, and launch scans — all from the Prowler Cloud UI.
...
<Note>
**Nothing is deployed into your tenant.** Unlike AWS Organizations onboarding, Azure onboarding creates no resources in Azure. Prowler reuses the service principal credential you provide as the credential of every subscription it onboards, so a single grant covers discovery and scanning.
</Note>
...
- Management Group hierarchies are read up to a bounded depth. Deeper tenants report an error at discovery — see [Troubleshooting](#troubleshooting).

### Blocked Subscriptions

A subscription is shown grayed out and cannot be selected when onboarding it would conflict with something Prowler already stores. Hover the subscription to see the reason:

| Reason | What it means |
|--------|---------------|
| `provider_linked_to_other_organization` | The subscription is already connected under a **different** Prowler organization. |
| `provider_linked_to_other_organization_node` | The subscription is already grouped under a different Management Group in Prowler — for example, it moved in Azure after it was onboarded. |
...
### No Management Group With That ID Was Found
...
The tenant root Management Group could not be read. Either the tenant ID is wrong, or the service principal cannot see the root. Confirm the tenant ID and list what the credential can reach:
