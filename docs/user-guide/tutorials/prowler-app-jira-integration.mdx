...
Prowler uses a private delivery marker to check whether Jira created an issue when a delivery result is uncertain. If Jira later returns exactly one matching issue, Prowler saves that link. If Jira returns no match, multiple matches, or a lookup error, Prowler keeps the reservation and does not retry automatically because another creation request could produce a duplicate issue. In the rare case where delivery stopped before Jira received the request, this reservation can remain blocked and require manual investigation.
