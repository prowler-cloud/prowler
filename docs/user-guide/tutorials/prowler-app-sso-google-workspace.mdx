...
- If `userType` does not match any existing role, Prowler App creates a new role with that name **with all permissions enabled**. A Prowler administrator can adjust its permissions afterwards through the [RBAC Management](/user-guide/tutorials/prowler-app-rbac) tab.
