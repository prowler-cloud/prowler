...
- If `userType` is not set, the user's existing roles are left unchanged. Users without an existing role receive the `read_only` role until a Prowler administrator assigns another role.
